<template>
  ...
  <div
    v-if="showErrorModal"
    class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-2 sm:p-4 md:p-6"
  >
    <!-- Background overlay -->
    <div
    ...
    ></div>
    <div
      class="bg-white rounded-lg shadow-lg w-full max-w-xs sm:max-w-sm mx-auto relative z-60 max-h-[95vh] flex flex-col"
    >
      <div
        class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 bg-red-600 text-white rounded-t-lg"
      >
      ...
      <div
        class="flex justify-end p-3 sm:p-4 border-t border-gray-200 bg-gray-50 rounded-b-lg"
      >
        <button
          @click="$emit('closeErrorModal')"
  ...
  <div
    v-if="showSetupMcModal"
    class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-2 sm:p-4 md:p-6"
  >
    <!-- Background overlay -->
    <div
      class="fixed inset-0 bg-black bg-opacity-50"
      @click="$emit('closeSetupMcModal')"
    ></div>
    <div
      class="bg-white rounded-lg shadow-lg w-full max-w-sm sm:max-w-3xl md:max-w-5xl lg:max-w-6xl mx-auto flex flex-col max-h-[95vh] relative z-60"
    >
      <div
        class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg"
      >
      ...
      <div class="p-4 sm:p-6 overflow-y-auto flex-grow bg-gray-50">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="flex items-center">
              ...
              <input
                ...
                :value="formData.ac"
                readonly
                class="ml-2 px-3 py-1 border border-gray-300 rounded text-sm bg-white flex-1"
              />
            </div>
            <div class="flex items-center">
              <input
                ...
                readonly
                class="px-3 py-1 border border-gray-300 rounded text-sm bg-white flex-1"
              />
            </div>
          </div>
        ...
        <div class="flex justify-between items-center mt-2">
          ...
          <div
            class="text-xs text-gray-600 self-center"
            v-if="selectedComponentIndex !== null"
          >
            Editing component:
            <span class="font-semibold">{{
              localComponents[selectedComponentIndex]?.c_num
            }}</span>
          </div>
        </div>
      </div>
      <!-- Modal Footer -->
      <div
        class="flex items-center justify-end gap-2 p-3 border-t border-gray-200 bg-gray-100 rounded-b-lg flex-shrink-0"
      ></div>
    </div>
  </div>
  ...
  >
    ...
    <!-- Modal content -->
    <div
      class="bg-white rounded-lg shadow-lg w-full max-w-sm sm:max-w-xl md:max-w-3xl lg:max-w-5xl xl:max-w-6xl mx-auto flex flex-col max-h-[95vh] relative z-60"
    >
      <!-- Modal header -->
      <div
        class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg"
      ...
      <div class="p-3 sm:p-4 md:p-5 overflow-y-auto flex-grow">
        ...
        <div class="mb-3 sm:mb-4 flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <div class="w-full sm:w-32 md:w-40">
            ...
            <select
              :value="mcsSortOrder"
              @input="
                $emit('update:mcsSortOrder', $event.target.value);
                $emit('fetchMcsData');
              "
              class="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-xs sm:text-sm focus:ring-blue-500 focus:border-blue-500"
            >
        ...
        <div
          v-else-if="mcsMasterCards.length === 0"
        ...
        <div
          v-else
          class="overflow-auto rounded-lg border border-gray-200 flex-1 min-h-0"
        >
          <table
            class="w-full divide-y divide-gray-200 text-xs hidden sm:table"
            id="masterCardDataTable"
            style="min-width: 650px"
          >
            <thead class="bg-gray-50 sticky top-0">
              <tr v-if="mcsSortOption === 'mc_model'">
                <th
                  class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap"
                >
                  Model
                </th>
                <th
              ...
              >
                <template v-if="mcsSortOption === 'mc_model'">
                  ...
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">
                    <span
                      :class="
                        mcs.status === 'Act' || mcs.status === 'Active'
                ...
                <template v-else-if="mcsSortOption === 'ext_dim_1'">
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">
                    {{ mcs.ext_dim_1 }}x{{ mcs.ext_dim_2 }}x{{ mcs.ext_dim_3 }}
                  </td>
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">{{ mcs.seq }}</td>
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">{{ mcs.comp }}</td>
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">{{ mcs.p_design }}</td>
                ...
                <template v-else-if="mcsSortOption === 'int_dim_1'">
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">
                    {{ mcs.int_dim_1 }}x{{ mcs.int_dim_2 }}x{{ mcs.int_dim_3 }}
                  </td>
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">{{ mcs.seq }}</td>
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">{{ mcs.comp }}</td>
                  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3">{{ mcs.p_design }}</td>
          ...
          <div class="sm:hidden space-y-2 p-1">
            <div
              ...
              :class="[
                ...
                !allowObsoleteSelection && isObsoleteStatus(mcs)
                  ? 'bg-red-50 text-gray-400 cursor-not-allowed border-gray-200'
                  : 'bg-white hover:bg-blue-50 cursor-pointer border-gray-200',
                selectedMcs?.seq === mcs.seq ? 'bg-blue-100 border-blue-500' : '',
              ]"
              @click="handleMcsRowClick(mcs)"
            >
              ...
              <div class="text-gray-700 font-medium truncate mb-1">
                {{ mcs.model || "" }}
              </div>
              <div class="flex flex-wrap gap-x-4 gap-y-0.5 text-gray-600 text-[11px]">
                <div v-if="mcs.part">
                  <span class="font-semibold">Part:</span> {{ mcs.part }}
                </div>
                <div v-if="mcs.comp">
                  <span class="font-semibold">Comp#:</span> {{ mcs.comp }}
                </div>
                <div v-if="mcs.p_design">
                  <span class="font-semibold">PD:</span> {{ mcs.p_design }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div
          v-if="mcsSortOption === 'part_no'"
          class="mt-4 border border-gray-400 rounded bg-white"
        >
      ...
      <div class="border-t border-gray-200 bg-gray-100 rounded-b-lg flex-shrink-0 p-3">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
          <div class="text-xs text-gray-500" v-if="mcsMasterCards.length > 0">
            {{ mcsMasterCards.length }} records found · Page {{ mcsCurrentPage }} of
            {{ mcsLastPage }}
          </div>
  ...
  >
    ...
    ></div>
    <div
      class="bg-white rounded-lg shadow-lg w-full max-w-sm sm:max-w-4xl md:max-w-6xl mx-auto flex flex-col max-h-[95vh] relative z-60"
    >
      <!-- Modal Header -->
      <div
        class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg"
      ...
      <div class="p-4 overflow-y-auto flex-grow bg-gray-50">
        ...
        <div class="border border-gray-400 rounded mb-4 bg-white">
          ...
          <div class="p-3">
            ...
            <div class="hidden lg:block">
              ...
              <div class="mb-3 flex items-start justify-between">
                <div class="flex flex-wrap items-center space-x-4 flex-1">
                  <div class="flex items-center">
                    ...
                    <input
                      type="text"
                      v-model="selectedProductDesign"
                      class="w-32 px-2 py-1 border border-gray-400 text-xs"
                      readonly
                    />
                    <button
                  ...
                  <div class="flex items-center">
                    ...
                    <input
                      type="text"
                      v-model="selectedPaperFlute"
                      class="w-20 px-2 py-1 border border-gray-400 text-xs"
                      readonly
                    />
                    <button
                  ...
                  <div class="flex items-center">
                    ...
                    <input
                      type="text"
                      v-model="partNo"
                      class="w-40 px-2 py-1 border border-gray-400 text-xs"
                    />
                  </div>
                  <div class="flex items-center">
                    <button
                      class="px-3 py-1 bg-blue-200 border border-gray-400 text-xs font-medium"
                      @click="openMoreDescriptionModal"
                    >
                      Description
                    </button>
                  </div>
                ...
                <div class="w-64 flex flex-col space-y-1">
                  <div class="flex items-center">
                    <label class="text-xs font-medium w-32 whitespace-nowrap"
                      >System Gross Area:</label
                    >
                    <div class="flex items-center">
                      <input
                        type="text"
              ...
              <!-- Row 2: B/Quality -->
              <div class="mb-3 flex items-start justify-between">
                <div class="bg-yellow-200 p-2 rounded flex-1">
                  <div class="flex flex-wrap items-center gap-1">
                    <label class="text-xs font-bold w-16">B/Quality:</label>
                    <input
                      type="text"
                      value="DB"
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-yellow-100"
                    />
                    <button
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs invisible"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                    ...
                    <input
                      type="text"
                      value="A/C/E"
                      class="w-16 px-1 py-1 border border-gray-400 text-xs text-center bg-yellow-100"
                    />
                    <button
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs invisible"
                    >
              ...
              <!-- Row 3: SO -->
              <div class="mb-3 flex items-start justify-between">
                <div class="p-2 rounded flex-1">
                  <div class="flex flex-wrap items-center gap-1">
                    <label class="text-xs font-bold w-16">SO:</label>
                    <input
                      type="text"
                      :value="getSoDisplay(0)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openPaperQualityModal', 0)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getSoDisplay(1)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openPaperQualityModal', 1)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getSoDisplay(2)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openPaperQualityModal', 2)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getSoDisplay(3)"
                      readonly
                      class="w-16 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openPaperQualityModal', 3)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getSoDisplay(4)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openPaperQualityModal', 4)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
              ...
              <div class="mb-3 flex items-start justify-between">
                <div class="p-2 rounded flex-1">
                  <div class="flex flex-wrap items-center gap-1">
                    ...
                    <input
                      type="text"
                      :value="getWoDisplay(0)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openWoPaperQualityModal', 0)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getWoDisplay(1)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openWoPaperQualityModal', 1)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getWoDisplay(2)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openWoPaperQualityModal', 2)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getWoDisplay(3)"
                      readonly
                      class="w-16 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openWoPaperQualityModal', 3)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                    <input
                      type="text"
                      :value="getWoDisplay(4)"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      @click="emit('openWoPaperQualityModal', 4)"
                      class="px-1 py-1 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                ...
                <div class="flex flex-wrap items-center space-x-4 flex-1">
                  <div class="flex items-center">
                    ...
                    <input
                      type="text"
                      v-model="idL"
                      class="w-16 px-2 py-1 border border-gray-400 text-xs"
                    />
                    <span class="text-xs ml-1">L</span>
                    <input
                      type="text"
                      v-model="idW"
                      class="w-16 px-2 py-1 border border-gray-400 text-xs ml-2"
                    />
                    ...
                    <span class="ml-2 px-2 py-1 bg-red-200 border border-red-400 text-xs"
                      >t2e</span
                    >
                  </div>
                  <div class="flex items-center">
                  ...
                  <div class="flex items-center">
                    ...
                    <input
                      type="text"
                      :value="selectedChemicalCoat"
                      readonly
                      class="w-24 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                    />
                    <button
                      class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                      @click="openChemicalCoatModal"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                  </div>
              ...
              <div class="mb-3 flex items-start justify-between">
                <div class="flex flex-wrap items-center space-x-4 flex-1">
                  ...
                  <div class="flex items-center">
                    ...
                    <input
                      type="text"
                      :value="selectedReinforcementTape"
                      readonly
                      class="w-28 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                    />
                    <button
                      class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                      @click="openReinforcementTapeModal"
                    >
                      <i class="fas fa-search"></i>
                    </button>
                  </div>
            ...
            <div class="block lg:hidden space-y-2">
              <!-- P/Design -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-20">P/design:</label>
                <input
                  type="text"
                  v-model="selectedProductDesign"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
                  readonly
                />
                <button
                  @click="openProductDesignModal"
              ...
              <!-- Flute -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-20">Flute:</label>
                <input
                  type="text"
                  v-model="selectedPaperFlute"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <!-- Part No -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-20">Part No:</label>
                <input
                  type="text"
                  v-model="partNo"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <!-- B/Quality, SO, WO Table -->
              <div class="overflow-x-auto">
                <table class="w-full text-xs border-collapse">
                  <thead>
                    <tr class="bg-yellow-200">
                      <th class="border border-gray-400 px-2 py-1 text-left font-bold">
                        B/quality
                      </th>
                      <th class="border border-gray-400 px-2 py-1 text-center font-bold">
                        SO
                      </th>
                      <th class="border border-gray-400 px-2 py-1 text-center font-bold">
                        WO
                  ...
                  <tbody>
                    <tr>
                      <td
                        class="border border-gray-400 px-2 py-1 bg-yellow-100 text-center"
                      >
                        DB
                      </td>
                      <td class="border border-gray-400 px-1 py-1">
                        <div class="flex items-center justify-center gap-1">
                          <input
                            ...
                            :value="getSoDisplay(0)"
                            readonly
                            class="w-16 px-1 py-0.5 border border-gray-400 text-xs text-center bg-gray-50"
                          />
                          <button
                            @click="emit('openPaperQualityModal', 0)"
                            class="px-1 py-0.5 bg-blue-500 border border-blue-600 text-xs hover:bg-blue-600 text-white"
                          >
                            <i class="fas fa-search"></i>
                          </button>
                        </div>
              ...
              <div class="flex items-center gap-2">
                ...
                <input
                  type="text"
                  v-model="idL"
                  class="w-14 px-2 py-1 border border-gray-400 text-xs"
                />
                ...
                <span class="px-2 py-1 bg-red-200 border border-red-400 text-xs"
                  >t2e</span
                >
              </div>
              ...
              <!-- Nest/Slot Row (inside PRODUCT section) -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-20">Nest/Slot:</label>
                <input
                  type="text"
                  v-model="nestSlot"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <!-- Chem Coat Row -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-20">Chem coat:</label>
                <input
                  type="text"
                  :value="selectedChemicalCoat"
                  readonly
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                />
                <button
                  class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                  @click="openChemicalCoatModal"
              ...
              <!-- RF Tape Row -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-20">RF tape:</label>
                <input
                  type="text"
                  :value="selectedReinforcementTape"
                  readonly
              ...
              <!-- Measurement Fields (Below RF Tape) -->
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      :value="formatTrimZeros((displayGrossM2PerPcs || 0).toFixed(6))"
                      readonly
                      class="w-24 px-2 py-1 border border-gray-400 text-xs text-right bg-gray-50"
                    />
                    <span class="text-xs ml-1">m2</span>
                  </div>
                </div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center">
                    <input
                      ...
                      :value="formatTrimZeros((displayGrossM2PerPcs || 0).toFixed(6))"
                      readonly
                      class="w-24 px-2 py-1 border border-gray-400 text-xs text-right"
                    />
                    <span class="text-xs ml-1">m2</span>
                  </div>
                </div>
        ...
        <div class="border border-gray-400 rounded mb-4 bg-white">
          ...
          <div class="p-3">
            ...
            <div class="hidden lg:block">
              <!-- Score L Row -->
              <div
                class="mb-2 space-y-2 lg:space-y-0 lg:flex lg:items-center lg:justify-between"
              >
                <div class="flex flex-wrap items-center gap-1">
                  ...
                  <div class="flex flex-wrap items-center gap-1">
                    <template v-for="i in 8" :key="'scoreL' + i">
                      <input
                        type="text"
                        v-model.number="scoreL[i - 1]"
                        class="w-12 px-1 py-1 border border-gray-400 text-xs text-center"
                      />
                  ...
                  />
                  <button
                    class="ml-2 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="calculateScoreL"
                  >
              ...
              <!-- P/Size Row -->
              <div
                class="mb-2 space-y-2 lg:space-y-0 lg:flex lg:flex-wrap lg:items-center lg:gap-4"
              >
                <div class="flex items-center">
                  ...
                  <input
                    ...
                    readonly
                    class="w-20 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                ...
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    v-model="convDuctX2A"
                    class="w-12 px-2 py-1 border border-gray-400 text-xs text-center"
                  />
                ...
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    :value="selectedScoringToolCode"
                    readonly
                    class="w-8 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="openScoringToolModal"
                    title="Select Scoring Tool"
                  >
                    <i class="fas fa-search"></i>
                  </button>
                </div>
            ...
            <div class="block lg:hidden space-y-3">
              ...
              <div class="space-y-2">
                <div class="flex items-start">
                  <label class="text-xs font-medium w-24 flex-shrink-0">Score L:</label>
                  <div class="flex flex-wrap items-center gap-1 flex-1">
                    <template v-for="i in 8" :key="'mobileScoreL' + i">
                      <input
                        type="text"
                        v-model.number="scoreL[i - 1]"
                        class="w-12 px-1 py-1 border border-gray-400 text-xs text-center"
                ...
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24">Total Score L:</label>
                  <input
                    type="text"
                    :value="scoreLTotal"
                    readonly
                    class="flex-1 px-2 py-1 border border-gray-400 text-xs text-right bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="calculateScoreL"
                ...
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24">Sheet Length:</label>
                  <input
                    type="text"
                    v-model="sheetLength"
                    class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <div class="space-y-2">
                <div class="flex items-start">
                  <label class="text-xs font-medium w-24 flex-shrink-0">Score W:</label>
                  <div class="flex flex-wrap items-center gap-1 flex-1">
                    <template v-for="i in 8" :key="'mobileScoreW' + i">
                      <input
                        type="text"
                        v-model.number="scoreW[i - 1]"
                        class="w-12 px-1 py-1 border border-gray-400 text-xs text-center"
                ...
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24">Total Score W:</label>
                  <input
                    type="text"
                    :value="scoreWTotal"
                    readonly
                ...
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24">Sheet Width:</label>
                  <input
                    type="text"
                    v-model="sheetWidth"
                    class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <div class="space-y-2">
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24">P/Size:</label>
                  <input
                    type="text"
                    :value="selectedPaperSize"
                    readonly
                ...
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24">S/Tool:</label>
                  <input
                    type="text"
                    :value="selectedScoringToolCode"
                    readonly
              ...
              <!-- Corrugating Values -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Corr. Out:</label>
                <input
                  type="text"
                  v-model="conOut"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <div class="flex items-center">
                ...
                <input
                  type="text"
                  v-model="convDuctX2A"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs text-center max-w-[4.5rem]"
                />
                <span class="mx-1 text-xs font-bold">x</span>
                <input
                  type="text"
              ...
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Pcs-to-Joint:</label>
                <input
                  type="text"
                  v-model="pcsToJoint"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
        ...
        <div class="border border-gray-400 rounded mb-4 bg-white">
          ...
          <div class="p-3">
            ...
            <div class="hidden lg:block">
              <!-- Print Color Row -->
              <div class="mb-2 space-y-2 lg:space-y-0 lg:flex lg:items-start">
                <label class="text-xs font-medium w-20 flex-shrink-0">Print Color:</label>
                <div class="flex flex-wrap gap-1">
                  ...
                  >
                    <input
                      type="text"
                      :value="printColorCodes[i - 1]"
                      readonly
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                    />
                    <button
                      class="w-8 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                      @click="openColorModal(i - 1)"
                      title="Select Color"
                    >
                      <i class="fas fa-search"></i>
                    </button>
              ...
              <div
                class="mb-2 space-y-2 lg:space-y-0 lg:flex lg:items-center lg:justify-between"
              >
                <div class="flex items-center flex-1 flex-wrap gap-1">
                  <label class="text-xs font-medium w-20">Print Area(%):</label>
                  <div class="flex flex-wrap gap-1">
                    <input
                      type="text"
                      v-model="colorAreaPercents[i - 1]"
                      class="w-12 px-1 py-1 border border-gray-400 text-xs text-center"
                      v-for="i in 7"
                      :key="'area' + i"
                    />
                ...
                <div class="flex items-center lg:w-64 w-full lg:mt-0 mt-1">
                  <label class="text-xs font-medium w-24">Prt Block#:</label>
                  <input
                    type="text"
                    v-model="pitBlockNo"
                    class="w-24 px-2 py-1 border border-gray-400 text-xs"
                  />
                </div>
              </div>
              ...
              >
                ...
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    :value="selectedGlueingCode"
                    readonly
                    class="w-20 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="openGlueingModal"
                    title="Select Glueing Material"
                  >
                    <i class="fas fa-search"></i>
                  </button>
                </div>
              ...
              >
                ...
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    :value="selectedStitchWireCode"
                    readonly
                    class="w-16 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="openStitchWireModal"
                    title="Select Stitch Wire"
                  >
                    <i class="fas fa-search"></i>
                  </button>
                  <input
                    type="text"
                    v-model="stitchWirePieces"
                    class="ml-2 w-16 px-2 py-1 border border-gray-400 text-xs text-right"
                    placeholder="pcs"
                  />
                </div>
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    :value="selectedWrappingCode"
                    readonly
                    class="w-20 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="openWrappingModal"
                    title="Select Wrapping Material"
                  >
                    <i class="fas fa-search"></i>
                  </button>
                </div>
              ...
              >
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    :value="selectedFinishingCode"
                    readonly
                    class="w-32 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="openFinishingModal"
                ...
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    :value="selectedBundlingStringCode"
                    readonly
                    class="w-16 px-2 py-1 border border-gray-400 text-xs bg-gray-50"
                  />
                  <button
                    class="ml-1 px-2 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                    @click="openBundlingStringModal"
                    title="Select Bundling String"
                  >
                    <i class="fas fa-search"></i>
                  </button>
                  <input
                    type="text"
                    v-model="bundlingStringQty"
                    class="ml-2 w-16 px-2 py-1 border border-gray-400 text-xs text-right"
                    placeholder="qty"
                  />
                </div>
                <div class="flex items-center">
                  ...
                  <input
                    type="text"
                    v-model="bdlPerPallet"
                    class="w-12 px-2 py-1 border border-gray-400 text-xs"
                  />
                </div>
                <div class="flex items-center lg:ml-auto">
                  <div class="flex items-center">
              ...
              >
                <div class="flex items-center lg:w-80 w-full">
                  <label class="text-xs font-medium w-20">Item Remark:</label>
                  <input
                    type="text"
                    v-model="itemRemark"
                    class="w-full lg:w-40 px-2 py-1 border border-gray-400 text-xs"
                  />
                </div>
                <div
                  class="flex items-center lg:w-64 w-full lg:justify-start justify-between mt-1 lg:mt-0"
                >
                  <label class="text-xs font-medium w-24">Peel Off%:</label>
                  <input
                    type="text"
                    v-model="peelOffPercent"
                    class="w-24 px-2 py-1 border border-gray-400 text-xs text-right"
                  />
                </div>
                <div class="flex items-center lg:ml-auto mt-1 lg:mt-0">
                  <button
                    class="px-3 py-1 bg-blue-500 hover:bg-blue-600 border border-blue-600 text-xs font-bold text-white transition-colors"
                    @click="openMspModal"
                  >
                    MSP
                  </button>
            ...
            <!-- MOBILE LAYOUT (< lg) -->
            <div class="block lg:hidden space-y-3">
              <!-- Print Color and Print Area - Two Columns -->
              <div class="grid grid-cols-2 gap-3">
                <!-- Print Color Column -->
                <div class="space-y-1 min-w-0">
                  <label class="text-xs font-bold block text-center">Print Color:</label>
                  <div class="space-y-1">
                    <div
                      v-for="i in 7"
                      :key="'mobileColor' + i"
                    ...
                    >
                      <input
                        ...
                        :value="printColorCodes[i - 1]"
                        readonly
                        class="flex-1 min-w-0 px-1 py-1 border border-gray-400 text-xs text-center bg-gray-50"
                      />
                      <button
                        class="flex-shrink-0 w-7 h-7 px-1 py-1 bg-gray-200 border border-gray-400 text-xs hover:bg-gray-300"
                        @click="openColorModal(i - 1)"
                        title="Select Color"
                      >
                ...
                <!-- Print Area Column -->
                <div class="space-y-1 min-w-0">
                  <label class="text-xs font-bold block text-center"
                    >Print Area(%):</label
                  >
                  <div class="space-y-1">
                    <input
                      type="text"
                      v-model="colorAreaPercents[i - 1]"
                      class="w-full min-w-0 px-1 py-1 border border-gray-400 text-xs text-center"
                      v-for="i in 7"
                      :key="'mobileArea' + i"
                    />
                  </div>
              ...
              <div class="flex items-center gap-1">
                <label class="text-xs font-medium w-24 flex-shrink-0">D/Cut Sheet:</label>
                <input
                  type="text"
                  v-model="dcutSheetL"
                  class="w-16 px-1 py-1 border border-gray-400 text-xs"
                />
                <span class="text-xs">L</span>
                <input
                  type="text"
              ...
              <!-- D/Cut Mould -->
              <div class="flex items-center gap-1">
                <label class="text-xs font-medium w-24 flex-shrink-0">D/Cut Mould:</label>
                <input
                  type="text"
                  v-model="dcutMouldL"
                  class="w-16 px-1 py-1 border border-gray-400 text-xs"
              ...
              <div class="flex items-center">
                ...
                <input
                  type="text"
                  v-model="peelOffPercent"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs text-right"
                />
              </div>

              <!-- Bdl/Pallet -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Bdl/Pallet:</label>
                <input
                  type="text"
                  v-model="bdlPerPallet"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <!-- Prt Block# -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Plt Block#:</label>
                <input
                  type="text"
                  v-model="pitBlockNo"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <div class="space-y-2">
                ...
                <div class="flex items-center">
                  <label class="text-xs font-medium w-24"></label>
                  <input
                    type="text"
                    v-model="stitchWirePieces"
                    class="flex-1 px-2 py-1 border border-gray-400 text-xs text-right"
              ...
              <!-- Finishing -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Finishing:</label>
                <input
                  type="text"
                  :value="selectedFinishingCode"
                  readonly
              ...
              <!-- Glueing -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Glueing:</label>
                <input
                  type="text"
                  :value="selectedGlueingCode"
                  readonly
              ...
              <!-- Wrapping -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Wrapping:</label>
                <input
                  type="text"
                  :value="selectedWrappingCode"
                  readonly
              ...
              <!-- Item Remark -->
              <div class="flex items-center">
                <label class="text-xs font-medium w-24">Item Remark:</label>
                <input
                  type="text"
                  v-model="itemRemark"
                  class="flex-1 px-2 py-1 border border-gray-400 text-xs"
              ...
              <div class="flex items-center justify-between gap-2 text-xs">
                <div class="flex items-center gap-1">
                  <label for="handHoleMobile" class="font-medium">Hand Hole:</label>
                  <input
                    type="checkbox"
                    id="handHoleMobile"
                    v-model="handHole"
                ...
                <div class="flex items-center gap-1">
                  <label for="rotaryDCutMobile" class="font-medium">Rotary D/Cut:</label>
                  <input
                    type="checkbox"
                    id="rotaryDCutMobile"
                    v-model="rotaryDCut"
                ...
                <div class="flex items-center gap-1">
                  <label for="fullBlockPrintMobile" class="font-medium"
                    >Full Block Print:</label
                  >
                  <input
                    type="checkbox"
                    id="fullBlockPrintMobile"
                    v-model="fullBlockPrint"
                    class="w-4 h-4"
                  />
                </div>
  ...
  <ChemicalCoatModal
    ...
    @select="
      (coat) => {
        selectedChemicalCoat = coat?.code || '';
  ...
  <ReinforcementTapeModal
    ...
    @select="
      (row) => {
        selectedReinforcementTape = row?.code || '';
        showReinforcementTapeModal = false;
      }
    "
  />
  ...
  <PaperSizeModal
    ...
    @select="
      (size) => {
        selectedPaperSize = size?.millimeter || '';
        showPaperSizeModal = false;
      }
    "
  />
  <ColorModal
  ...
  <MCSpecialInstruction
    ...
    @update:value="
      (v) => {
        specialInstructions.value = Array.isArray(v) ? v.slice(0, 4) : [];
      }
    "
...
const fetchProductDesigns = async () => {
  try {
    ...
    if (response.ok) {
      ...
      productDesigns.value = Array.isArray(data)
        ? data
...
const fetchPaperSizes = async () => {
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      paperSizeRows.value = Array.isArray(data)
        ? data
        : Array.isArray(data?.data)
        ? data.data
        : [];
...
const fetchWrappingMaterials = async () => {
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      wrappingItems.value = Array.isArray(data)
        ? data
        : Array.isArray(data?.data)
        ? data.data
        : [];
...
const openMoreDescriptionModal = () => {
  try {
    console.debug(
      "[UpdateMcModal] Opening MoreDescriptionModal, current moreDescriptions:",
      JSON.parse(JSON.stringify(moreDescriptions.value))
    );
  } catch (e) {
...
const onMoreDescriptionSave = (rows) => {
  ...
  try {
    console.debug(
      "[UpdateMcModal] MoreDescription saved:",
...
const mcGrossKgPerSet = computed(() => {
  ...
  const selectedFlute = props.paperFlutes?.find(
    (f) => f.code === selectedPaperFlute.value || f.Flute === selectedPaperFlute.value
  );
...
const hydratePdFromObject = (pd, loaded) => {
  ...
  selectedProductDesign.value =
    pd.selectedProductDesign || loaded.P_DESIGN || loaded.p_design || "";
  ...
  printColorCodes.value = Array.isArray(pd.printColorCodes)
    ? pd.printColorCodes
    : printColorCodes.value;
  colorAreaPercents.value = Array.isArray(pd.colorAreaPercents)
    ? pd.colorAreaPercents
    : colorAreaPercents.value;
  ...
  stitchWirePieces.value = formatTrimZeros(
    pd.stitchWirePieces || (loaded.SWIRE_PCS ?? "")
  );
  ...
  stitchWirePieces.value = formatTrimZeros(
    pd.stitchWirePieces || stitchWirePieces.value || ""
  );
  selectedBundlingStringCode.value =
    pd.selectedBundlingStringCode || loaded.STRING_TYPE || "";
  bundlingStringQty.value = formatTrimZeros(
    pd.bundlingStringQty || (loaded.PCS_PER_BLD ?? "")
  );
...
const mcComponentsToRender = computed(() => {
  ...
  if (!Array.isArray(fromLoaded) || fromLoaded.length === 0) {
    if (
      props.mcLoaded?.mc_seq ||
      props.mcLoaded?.mcs ||
      Array.isArray(props.mcComponents)
    ) {
...
watch(
  mcComponentsToRender,
  (rows) => {
    // Skip if components already loaded from database
    if (componentsLoadedFromDb.value) {
      console.log(
        "⏭️ Skipping mcComponentsToRender update - components already loaded from DB",
        {
          flag: componentsLoadedFromDb.value,
          currentLocalComponents: localComponents.value.map((c) => ({
            c_num: c.c_num,
            pd: c.pd,
          })),
        }
...
watch(
  () => props.showSetupMcModal,
  async (newVal) => {
    if (newVal && props.mcLoaded) {
      console.log(
        "🔍 Setup MC Modal opened with existing MC, fetching components from database"
      );
...
watch(
  ...
  (vals) => {
    try {
      ...
      const base = Array.isArray(current.soValues)
        ? current.soValues.slice(0, 5)
        : ["", "", "", "", ""];
...
watch(
  ...
  (vals) => {
    try {
      const idx = selectedComponentIndex.value ?? 0;
      const current = componentForms.value[idx] || makeEmptyPdState();
      const base = Array.isArray(current.woValues)
        ? current.woValues.slice(0, 5)
        : ["", "", "", "", ""];
      const incoming = Array.isArray(vals) ? vals : [];
      const merged = base.map((v, i) => {
        const nv = incoming[i];
  ...
  (loaded) => {
    ...
    if (loaded?.pd_setup) {
      ...
      for (let idx = 0; idx < componentForms.value.length; idx++) {
        ...
        if (compSrc) {
          // Core identifiers
          cf.selectedProductDesign =
            compSrc.selectedProductDesign ||
            compSrc.pd ||
            compSrc.p_design ||
            cf.selectedProductDesign;
          cf.pcsPerSet =
            compSrc.pcsPerSet || compSrc.pcs_set || compSrc.pcs || cf.pcsPerSet;
          ...
          const soArr = Array.isArray(compSrc.soValues)
            ? compSrc.soValues
            : Array.isArray(compSrc.so)
            ? compSrc.so
            : fromNumberedKeys(compSrc, "so");
          const woArr = Array.isArray(compSrc.woValues)
            ? compSrc.woValues
            : Array.isArray(compSrc.wo)
            ? compSrc.wo
            : fromNumberedKeys(compSrc, "wo");
          cf.soValues =
            Array.isArray(cf.soValues) && cf.soValues.some((v) => v)
              ? cf.soValues
              : normalize5(soArr);
          cf.woValues =
            Array.isArray(cf.woValues) && cf.woValues.some((v) => v)
              ? cf.woValues
              : normalize5(woArr);
          ...
          // Colors
          const pcArr = Array.isArray(compSrc.printColorCodes)
            ? compSrc.printColorCodes
            : [];
          cf.printColorCodes =
            Array.isArray(cf.printColorCodes) && cf.printColorCodes.some((v) => v)
              ? cf.printColorCodes
              : pcArr.slice(0, 7).concat(["", "", "", "", "", ""]).slice(0, 7);
          const caArr = Array.isArray(compSrc.colorAreaPercents)
            ? compSrc.colorAreaPercents
            : [];
          cf.colorAreaPercents =
            Array.isArray(cf.colorAreaPercents) && cf.colorAreaPercents.some((v) => v)
              ? cf.colorAreaPercents
          ...
          cf.selectedReinforcementTape =
            compSrc.selectedReinforcementTape ?? cf.selectedReinforcementTape;
          cf.selectedChemicalCoat =
            compSrc.selectedChemicalCoat ?? cf.selectedChemicalCoat;
          // Corrugating / converting values per component
          cf.selectedPaperFlute = compSrc.selectedPaperFlute ?? cf.selectedPaperFlute;
          cf.selectedScoringToolCode =
            compSrc.selectedScoringToolCode ?? cf.selectedScoringToolCode;
          ...
          cf.selectedStitchWireCode =
            compSrc.selectedStitchWireCode ?? cf.selectedStitchWireCode;
          cf.bundlingStringQty = compSrc.bundlingStringQty ?? cf.bundlingStringQty;
          cf.selectedBundlingStringCode =
            compSrc.selectedBundlingStringCode ?? cf.selectedBundlingStringCode;

          // Glueing / wrapping / finishing
          cf.selectedGlueingCode = compSrc.selectedGlueingCode ?? cf.selectedGlueingCode;
          cf.selectedWrappingCode =
            compSrc.selectedWrappingCode ?? cf.selectedWrappingCode;
          cf.bdlPerPallet = compSrc.bdlPerPallet ?? cf.bdlPerPallet;
          cf.selectedFinishingCode =
            compSrc.selectedFinishingCode ?? cf.selectedFinishingCode;
    ...
    try {
      const noComp =
        !loaded?.pd_setup ||
        !Array.isArray(loaded.pd_setup?.components) ||
        loaded.pd_setup.components.length === 0;
      if (noComp) {
        const cf0 = componentForms.value[0] || makeEmptyPdState();
        cf0.selectedProductDesign =
          cf0.selectedProductDesign || loaded.P_DESIGN || loaded.p_design || "";
...
const fetchMcComponentsFromDb = async () => {
  try {
    ...
    const response = await fetch(
      `/api/update-mc/master-cards/${mcsSeqEnc}/components?customer_code=${custEnc}`,
      {
        headers: {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      for (let i = 0; i < COMPONENT_LABELS.length; i++) {
        ...
        if (fetchedComp) {
          ...
          // Core identifiers
          cf.selectedProductDesign =
            fetchedComp.selectedProductDesign ||
            fetchedComp.pd ||
            fetchedComp.p_design ||
            cf.selectedProductDesign;
          cf.pcsPerSet =
            fetchedComp.pcsPerSet ||
            fetchedComp.pcs_set ||
            fetchedComp.pcs ||
            cf.pcsPerSet;
          cf.partNo =
            fetchedComp.partNo ||
            fetchedComp.part_num ||
            fetchedComp.part_no ||
            cf.partNo;
          ...
          const soArr = Array.isArray(fetchedComp.soValues)
            ...
            : Array.isArray(fetchedComp.so)
            ? fetchedComp.so
            : fromNumberedKeysLocal(fetchedComp, "so");
          const woArr = Array.isArray(fetchedComp.woValues)
            ? fetchedComp.woValues
            : Array.isArray(fetchedComp.wo)
            ? fetchedComp.wo
            : fromNumberedKeysLocal(fetchedComp, "wo");
          cf.soValues =
            Array.isArray(cf.soValues) && cf.soValues.some((v) => v)
              ? cf.soValues
              : normalize5Local(soArr);
          cf.woValues =
            Array.isArray(cf.woValues) && cf.woValues.some((v) => v)
              ? cf.woValues
              : normalize5Local(woArr);
          ...
          // Colors
          const pcArr = Array.isArray(fetchedComp.printColorCodes)
            ? fetchedComp.printColorCodes
            : [];
          cf.printColorCodes =
            Array.isArray(cf.printColorCodes) && cf.printColorCodes.some((v) => v)
              ? cf.printColorCodes
              : pcArr.slice(0, 7).concat(["", "", "", "", "", ""]).slice(0, 7);
          const caArr = Array.isArray(fetchedComp.colorAreaPercents)
            ? fetchedComp.colorAreaPercents
            : [];
          cf.colorAreaPercents =
            Array.isArray(cf.colorAreaPercents) && cf.colorAreaPercents.some((v) => v)
              ? cf.colorAreaPercents
          ...
          cf.selectedReinforcementTape =
            fetchedComp.selectedReinforcementTape ?? cf.selectedReinforcementTape;
          cf.selectedChemicalCoat =
            fetchedComp.selectedChemicalCoat ?? cf.selectedChemicalCoat;
          // Corrugating / converting values per component
          cf.selectedPaperFlute = fetchedComp.selectedPaperFlute ?? cf.selectedPaperFlute;
          cf.selectedScoringToolCode =
            fetchedComp.selectedScoringToolCode ?? cf.selectedScoringToolCode;
          ...
          cf.selectedStitchWireCode =
            fetchedComp.selectedStitchWireCode ?? cf.selectedStitchWireCode;
          cf.bundlingStringQty = fetchedComp.bundlingStringQty ?? cf.bundlingStringQty;
          cf.selectedBundlingStringCode =
            fetchedComp.selectedBundlingStringCode ?? cf.selectedBundlingStringCode;

          // Glueing / wrapping / finishing
          cf.selectedGlueingCode =
            fetchedComp.selectedGlueingCode ?? cf.selectedGlueingCode;
          cf.selectedWrappingCode =
            fetchedComp.selectedWrappingCode ?? cf.selectedWrappingCode;
          cf.bdlPerPallet = fetchedComp.bdlPerPallet ?? cf.bdlPerPallet;
          cf.selectedFinishingCode =
            fetchedComp.selectedFinishingCode ?? cf.selectedFinishingCode;
      ...
      console.log(
        "📊 Before update - localComponents:",
        localComponents.value.map((c) => ({
          c_num: c.c_num,
          pd: c.pd,
          pcs_set: c.pcs_set,
        }))
      ...
      console.log(
        "✅ componentForms hydrated from MC components:",
        nextComponentForms.map((cf, idx) => ({
          idx,
          partNo: cf?.partNo,
...
const openSetupPd = () => {
  ...
  if (isTrulyEmpty) {
    ...
    selectedProductDesign.value =
      cf.selectedProductDesign || row?.pd || selectedProductDesign.value || "";
    pcsPerSet.value = formatTrimZeros(
      cf.pcsPerSet || row?.pcs_set || pcsPerSet.value || ""
    );
    ...
    if (Array.isArray(cf.printColorCodes)) {
      printColorCodes.value = cf.printColorCodes
        .map((v) => (v ?? "").toString())
        .slice(0, 7)
        .concat(["", "", "", "", "", ""])
        .slice(0, 7);
    ...
    if (Array.isArray(cf.colorAreaPercents)) {
      colorAreaPercents.value = cf.colorAreaPercents
        .map((v) => formatTrimZeros(v ?? ""))
        .slice(0, 7)
        .concat(["", "", "", "", "", ""])
        .slice(0, 7);
    ...
    // Stitch / bundling
    stitchWirePieces.value = formatTrimZeros(
      cf.stitchWirePieces ?? stitchWirePieces.value ?? ""
    );
    selectedStitchWireCode.value = (
      cf.selectedStitchWireCode ??
      selectedStitchWireCode.value ??
    ...
    bundlingStringQty.value = formatTrimZeros(
      cf.bundlingStringQty ?? bundlingStringQty.value ?? ""
    );
    selectedBundlingStringCode.value = (
      cf.selectedBundlingStringCode ??
      selectedBundlingStringCode.value ??
    ...
    peelOffPercent.value = formatTrimZeros(
      cf.peelOffPercent ?? peelOffPercent.value ?? ""
    );
...
const applyPdToSelectedComponent = () => {
  ...
  next[idx] = {
    ...
    c_num:
      next[idx]?.c_num ||
      ["Main", "Fit1", "Fit2", "Fit3", "Fit4", "Fit5", "Fit6", "Fit7", "Fit8", "Fit9"][
        idx
      ],
...
const buildPdSetupPayload = () => {
  ...
  const rootSo = normalizeFive(
    soFromMain && soFromMain.some((v) => v)
      ? soFromMain
      : soFromProps && soFromProps.some?.((v) => v)
      ? soFromProps
      : soFromMc
  );
  const rootWo = normalizeFive(
    woFromMain && woFromMain.some((v) => v)
      ? woFromMain
      : woFromProps && woFromProps.some?.((v) => v)
      ? woFromProps
      : woFromMc
  );
  return {
