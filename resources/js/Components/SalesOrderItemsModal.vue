                ...
                <div class="p-4">
                    ...
                    <div class="border border-gray-300 rounded-md overflow-x-auto mb-4">
                      ...
                      <thead class="bg-blue-50">
                        <tr>
                          ...
                          <th class="px-2 py-2 text-left text-xs font-semibold uppercase tracking-wider">D/Order Ref.#</th>
                          <th class="px-2 py-2 text-right text-xs font-semibold uppercase tracking-wider">Amount</th>
                        </tr>
                      </thead>
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-if="soList.length === 0" class="bg-yellow-100">
                          <td class="px-2 py-2 text-sm font-medium">1001</td>
                          <td class="px-2 py-2 text-sm">{{ doData?.so_number || soNumber || '-' }}</td>
                          <td class="px-2 py-2 text-sm">{{ doData?.mc_seq || mcSeq || '-' }}</td>
                          <td class="px-2 py-2 text-sm">{{ doData?.do_ref || doData?.do_number || doNumber }}</td>
                          <td class="px-2 py-2 text-sm text-right font-semibold">{{ formatCurrency(doData?.total_amount || totalAmount) }}</td>
                        </tr>
                        <tr v-for="(item, index) in soList" :key="index" :class="index === 0 ? 'bg-yellow-100' : ''">
                          <td class="px-2 py-2 text-sm font-medium">{{ 1001 + index }}</td>
                          ...
                          <td class="px-2 py-2 text-sm text-right font-semibold">{{ formatCurrency(item.amount) }}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div class="mb-4 text-xs">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="font-medium text-blue-700">{{ doData?.model || model }}</span>
                    </div>
                  </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-xs">
                      <thead class="bg-gray-100">
                        <tr>
                          ...
                          <th class="px-2 py-2 text-center font-semibold text-gray-700">Unbill</th>
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        >
                          <td class="px-2 py-2 font-medium" :class="item.item !== 'Main' ? 'text-gray-400' : ''">{{ item.item }}</td>
                          <td class="px-2 py-2" :class="item.p_design === '-' ? 'text-gray-400' : ''">{{ item.p_design }}</td>
                          <td class="px-2 py-2 text-center" :class="item.pcs === null ? 'text-gray-400' : ''">{{ formatNumber(item.pcs) }}</td>
                          <td class="px-2 py-2 text-center" :class="item.unit === null ? 'text-gray-400' : ''">{{ item.unit || '-' }}</td>
                          <td class="px-2 py-2 text-right" :class="item.u_price === null ? 'text-gray-400' : ''">{{ formatCurrency(item.u_price) }}</td>
                          <td class="px-2 py-2 text-center" :class="item.deliver === null ? 'text-gray-400' : ''">{{ formatNumber(item.deliver) }}</td>
                          <td class="px-2 py-2 text-center text-gray-400">{{ formatNumber(item.reject) }}</td>
                          <td class="px-2 py-2 text-center" :class="item.unbill === null ? 'text-gray-400' : ''">{{ formatNumber(item.unbill) }}</td>
                          <td class="px-2 py-2 text-center">
                            <input 
                              v-if="item.item === 'Main'"
                          ...
                          <td class="px-2 py-2 text-center">
                            <span v-if="item.item === 'Main'">{{ formatNumber(item.to_bill_kg, 2) }}</span>
                            <span v-else class="text-gray-400">-</span>
                          </td>
                        </tr>
                      </tbody>
...
const onToBillChange = (item) => {
  ...
  // Calculate To Bill KG based on To Bill quantity
  // Formula: To Bill KG = To Bill Qty × KG per Unit
  // This gives us accurate weight proportional to quantity
  if (originalKgPerUnit.value > 0) {
    // Direct calculation: Quantity × KG per unit
    item.to_bill_kg = item.to_bill * originalKgPerUnit.value
    
    console.log('🔢 KG Calculation:', {
      to_bill: item.to_bill,
      kg_per_unit: originalKgPerUnit.value.toFixed(4),
      calculated_kg: item.to_bill_kg.toFixed(2),
      formula: `${item.to_bill} × ${originalKgPerUnit.value.toFixed(4)} = ${item.to_bill_kg.toFixed(2)}`
    })
  } else {
    item.to_bill_kg = 0
    console.error('❌ Cannot calculate KG: originalKgPerUnit is 0 or not set', {
      originalKgPerUnit: originalKgPerUnit.value,
      doData: doData.value ? 'loaded' : 'not loaded',
      item: item
    })
    
    // Try to get from item's kg_per_unit if available
    if (item.kg_per_unit && item.kg_per_unit > 0) {
      item.to_bill_kg = item.to_bill * item.kg_per_unit
      console.warn('⚠️ Using item.kg_per_unit as fallback:', item.kg_per_unit)
    }
  }
  ...
  console.log('✅ Updated:', {
    to_bill: item.to_bill,
    to_bill_kg: item.to_bill_kg.toFixed(2),
