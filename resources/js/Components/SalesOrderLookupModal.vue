  ...
  try {
    ...
    if (response.data.success) {
      const salesOrdersData = response.data.data.map(order => ({
        ...
        status: order.STS,
        delivery_location: order.D_LOC_Num || 'Same',
        customer_name: order.AC_NAME,
        master_card_model: order.MODEL,
        order_mode: order.TYPE,
        salesperson_name: order.SLM,
        order_group: order.GROUP_,
        order_type: order.TYPE,
        unit: order.UNIT,
        order_quantity: order.SO_QTY?.toLocaleString() || '0',
        net_delivery: '',
        balance: order.SO_QTY?.toLocaleString() || '0',
        amount: order.AMOUNT,
        po_date: order.PO_DATE,
        so_date: order.SO_DMY,
        items: [
