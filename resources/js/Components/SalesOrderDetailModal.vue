<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 overflow-y-auto">
    ...
    <div class="flex min-h-full items-center justify-center p-4">
      <div
        class="relative w-full max-w-7xl bg-white rounded-xl shadow-2xl flex flex-col max-h-[calc(100vh-2rem)] overflow-hidden"
      >
        <!-- Header -->
        <div
          class="flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-xl"
        >
        ...
        <div class="flex-1 overflow-y-auto p-4 bg-gray-50">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div class="space-y-3">
                <h4
                  class="text-sm font-semibold text-gray-700 border-b border-gray-200 pb-1"
                >
                  Order Information
                </h4>
                <div class="space-y-2">
                  ...
                  <div class="flex items-center justify-between">
                    <span class="text-xs text-gray-600 w-20 flex-shrink-0"
                      >S/Order#:</span
                    >
                    <div class="flex items-center space-x-1 flex-1">
                      <input
                        ...
                        type="text"
                        class="w-8 px-1 py-1 text-xs border border-gray-300 rounded"
                        placeholder="MM"
                      />
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <input
                      ...
                      type="text"
                      class="w-28 px-2 py-1 text-xs border border-gray-300 rounded"
                    />
                  </div>
                  <div class="flex items-center justify-between">
                    <span class="text-xs text-gray-600 w-20 flex-shrink-0"
              ...
              <div
                class="space-y-3 bg-gradient-to-br from-yellow-50 to-orange-50 p-3 rounded-lg border-2 border-yellow-300 shadow-sm"
              >
                ...
                <div class="space-y-3">
                  <div class="flex items-center justify-between">
                    <span class="text-xs text-yellow-700 w-24 flex-shrink-0 font-medium"
                      >To Delivery Set:</span
                    >
                      ...
                      <input
                        ...
                        :class="{
                          'border-red-500 bg-red-50':
                            (isToDeliverySetRequired && !orderDetail.toDeliverySet) ||
                            isToDeliverySetExceed,
                        }"
                  ...
                  <div class="flex items-center justify-between">
                    <span class="text-xs text-yellow-700 w-24 flex-shrink-0"
                      >Options:</span
                    >
                    <div class="flex items-center space-x-3">
                      <div class="flex items-center space-x-1">
          ...
          >
            <div
              class="bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-2 border-b border-gray-200"
            >
            ...
            <div class="overflow-x-auto max-h-96">
              <table class="w-full text-xs table-fixed min-w-[1200px]">
                <thead class="bg-gray-100 sticky top-0 z-10">
                  <tr>
                    <th
                      class="px-2 py-1.5 text-left font-medium text-gray-700 border-r border-gray-300"
                    >
                      Item
                    </th>
                    <th
                    ...
                    <th
                      class="px-2 py-1.5 text-center font-medium text-gray-700 border-r border-gray-300"
                    >
                      Pcs
                    </th>
                    <th
                    ...
                    <th
                      class="px-2 py-1.5 text-right font-medium text-gray-700 border-r border-gray-300"
                    >
                      Order
                    </th>
                    <th
                    ...
                    <th
                      class="px-2 py-1.5 text-right font-medium text-gray-700 border-r border-gray-300 bg-yellow-50 w-[140px] min-w-[140px]"
                    >
                      To Deliver <span class="text-red-500">*</span>
                    </th>
                    <th
                      class="px-2 py-1.5 text-right font-medium text-gray-700 bg-gray-50 w-[140px] min-w-[140px]"
                    >
                      Deliver KG
                    </th>
                  </tr>
                ...
                <tbody class="divide-y divide-gray-200">
                  ...
                  >
                    <td
                      class="px-2 py-1.5 font-medium text-gray-900 border-r border-gray-200"
                    >
                      {{ item.name }}
                    </td>
                    ...
                    <td
                      class="px-2 py-1.5 text-right border-r border-gray-200 bg-yellow-50 w-[140px] min-w-[140px]"
                    >
                      <input
                        ...
                        :disabled="
                          ...
                          (item.deliverKG !== '' &&
                            item.deliverKG !== null &&
                            item.deliverKG !== undefined)
                        "
                      />
                    </td>
                    <td class="px-2 py-1.5 text-right bg-gray-50 w-[140px] min-w-[140px]">
                      <input
                        v-if="isKgUnit(item)"
                        ...
                        :disabled="
                          item.toDeliver !== '' &&
                          item.toDeliver !== null &&
                          item.toDeliver !== undefined
                        "
                        @input="handleDeliverKgChange(item)"
                      />
          ...
          <div class="space-y-2 mt-4">
            <!-- Validation Warning -->
            <div
              v-if="isToDeliverySetRequired"
              class="bg-red-50 border-l-4 border-red-400 p-3 rounded"
            >
              <div class="flex items-center">
                <i class="fas fa-exclamation-triangle text-red-400 mr-2"></i>
                <p class="text-sm text-red-800 font-medium">
                  Please fill in "To Delivery Set" or at least one "To Deliver" quantity
                  to proceed
                </p>
              </div>
            </div>
            ...
            <!-- Exceed Warning -->
            <div
              v-if="isToDeliverySetExceed"
              class="bg-orange-50 border-l-4 border-orange-400 p-3 rounded"
            >
              <div class="flex items-center">
                <i class="fas fa-exclamation-triangle text-orange-400 mr-2"></i>
                <p class="text-sm text-orange-800 font-medium">
                  To Delivery Set cannot exceed Balance ({{
                    maxDeliverable.toLocaleString()
                  }})
                </p>
              </div>
            </div>
        ...
        <!-- Footer -->
        <div
          class="flex items-center justify-between p-4 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100 rounded-b-xl"
        >
...
const handleToDeliverManualChange = (item) => {
  ...
  // Mutual exclusivity: when To Deliver is used, Deliver KG must be empty
  if (item) {
    item.deliverKG = "";
  }
};

const handleDeliverKgChange = (item) => {
  if (!item) return;
  // Mutual exclusivity: when Deliver KG is used, To Deliver must be empty
  item.toDeliver = "";
};
...
const handleSave = () => {
  ...
  const hasToDeliverItems = itemRows.value.some((item) => {
    ...
    const deliverKgValue = parseFloat(item.deliverKG) || 0;
    return toDeliverValue > 0 || deliverKgValue > 0;
  });

  // Validasi: Harus ada To Delivery Set atau To Deliver yang diisi
  if (toDeliverySetValue <= 0 && !hasToDeliverItems) {
    error(
      'Please fill in "To Delivery Set" or at least one "To Deliver" quantity before proceeding'
    );
    return;
  }
...
const totalToDeliver = computed(() => {
  return itemRows.value.reduce((total, item) => {
    ...
    const deliverKg = parseFloat(item.deliverKG) || 0;
    return total + toDeliver + deliverKg;
  }, 0);
});
...
const isToDeliverySetExceed = computed(() => {
  const val =
    Number((orderDetail.toDeliverySet || "0").toString().replace(/,/g, "")) || 0;
  ...
  (newValue, oldValue) => {
    ...
    itemRows.value.forEach((item) => {
      ...
      if (item.pDesign && item.pDesign.trim() !== "") {
        ...
        if (isKgUnit(item)) {
          item.deliverKG = calculatedToDeliver > 0 ? calculatedToDeliver.toString() : "";
          item.toDeliver = "";
        } else {
...
watchEffect(() => {
  if (!isUpdatingFromDeliverySet.value) {
    ...
    if (mainItem) {
      const mainToDeliver = isKgUnit(mainItem)
        ? parseFloat(mainItem.deliverKG) || 0
        : parseFloat(mainItem.toDeliver) || 0;
      ...
      if (Math.abs(calculatedDeliverySet - currentDeliverySet) > 0.01) {
        orderDetail.toDeliverySet =
          calculatedDeliverySet > 0 ? calculatedDeliverySet.toString() : "0";
      }
    }
  }
});
...
async function hydrateFromSalesOrderData() {
  ...
  const soNo =
    props.salesOrderData.soNumber ||
    props.salesOrderData.selectedOrderData?.soNumber ||
    "";
  if (soNo) {
  ...
  try {
    if (soNo) {
      ...
      if (response.data?.success) {
        ...
        orderDetail.mcardSeq =
          data.order_info?.mcard_seq ||
          props.salesOrderData.selectedOrderData?.mcardSeq ||
          "";
        orderDetail.pOrderRef =
          data.order_info?.customer_po_number ||
          props.salesOrderData.selectedOrderData?.pOrderRef ||
          "";
        ...
        if (main) {
          ...
          if (apiBalance !== undefined && apiBalance !== null && apiBalance !== "") {
            ...
            const orderQty =
              Number((main.order || "0").toString().replace(/,/g, "")) || 0;
            const netDel =
              Number((main.delivery || "0").toString().replace(/,/g, "")) || 0;
            const bal = orderQty - netDel;
          ...
          data.fittings.forEach((fitting, idx) => {
            if (idx < 9) {
              ...
              if (row) {
                ...
                const orderQty =
                  Number((row.order || "0").toString().replace(/,/g, "")) || 0;
                const netDel =
                  Number((row.delivery || "0").toString().replace(/,/g, "")) || 0;
                const bal = orderQty - netDel;
                row.balance =
                  fitting.balance !== undefined &&
                  fitting.balance !== null &&
                  fitting.balance !== ""
                    ? fitting.balance
                    : Number.isFinite(bal)
                    ? bal.toString()
...
function hydrateFromSelectedOrder() {
  ...
  if (Array.isArray(selectedOrder.items)) {
    ...
    const findVal = (name) =>
      selectedOrder.items.find((i) => i.name === name)?.pDesign || "";
    if (main) {
