<template>
  <div
    v-if="show"
    class="fixed inset-0 z-100 flex items-center justify-center overflow-y-auto p-2 sm:p-4 md:p-6"
  >
    <!-- Background overlay -->
    ...
    <!-- Modal content -->
    <div
      class="bg-white rounded-lg shadow-lg w-full max-w-sm sm:max-w-xl md:max-w-3xl lg:max-w-5xl xl:max-w-6xl z-110 relative max-h-[95vh] flex flex-col"
    >
      <!-- Modal Header -->
      <div
      ...
      >
        <div class="flex items-center">
          ...
          <h3 class="text-base sm:text-xl font-semibold truncate">
      ...
      <div class="p-3 sm:p-4 md:p-5 flex-1 overflow-hidden flex flex-col">
        ...
        <div
          v-else-if="error"
          class="p-4 text-red-500 bg-red-50 rounded border border-red-200 text-xs sm:text-sm"
        >
        ...
        <div
          ...
          class="p-4 text-center text-gray-500 text-xs sm:text-sm"
        >
          <p>No accounts found.</p>
          <div class="mt-1 hidden sm:block">Total loaded: {{ allAccounts.length }}</div>
        </div>
        ...
        >
          <table
            class="w-full divide-y divide-gray-200 hidden sm:table"
            id="customerAccountDataTable"
            ...
            <thead class="bg-gray-50 sticky top-0">
              <tr>
                <th
                  ...
                  class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer whitespace-nowrap"
                  style="width: 15%"
                >
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-xs">
              <tr
                ...
                :class="[
                  ...
                  selectedAccount &&
                  selectedAccount.customer_code === account.customer_code
              ...
              >
                <td
                  class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 whitespace-nowrap font-medium text-gray-900 text-xs"
                >
                  {{ account.customer_code }}
                </td>
                <td
                  class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 whitespace-nowrap text-gray-700 text-xs"
                >
                  <div
                    class="max-w-[200px] sm:max-w-none truncate"
          ...
          <div class="sm:hidden space-y-2 p-1">
            <div
              ...
              :class="[
                ...
                selectedAccount && selectedAccount.customer_code === account.customer_code
            ...
            >
              ...
              <div
                class="text-gray-700 font-medium truncate mb-1"
                :title="account.customer_name"
              >
                {{ account.customer_name }}
              </div>
              <div class="flex flex-wrap gap-x-4 gap-y-0.5 text-gray-600 text-[11px]">
                <div>
                  <span class="font-semibold">S/P:</span>
                  {{ account.salesperson_code || "-" }}
                </div>
                <div>
                  <span class="font-semibold">Curr:</span>
                  {{ account.currency_code || "-" }}
                </div>
              </div>
            </div>
        ...
        <div class="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2">
          <button
            type="button"
            @click="sortTable('customer_code')"
            class="py-1.5 sm:py-2 px-2 sm:px-3 bg-gray-100 border border-gray-400 hover:bg-gray-200 text-xs rounded-md sm:rounded-lg transform active:translate-y-px transition-all"
          >
            <i class="fas fa-sort mr-1"></i
            ><span class="hidden lg:inline">Sort </span>Code
          </button>
          <button
            type="button"
            @click="sortTable('customer_name')"
          ...
          >
            <i class="fas fa-sort mr-1"></i
            ><span class="hidden lg:inline">Sort </span>Name
          </button>
          <button
            type="button"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isAccountSelectable = (account) => {
      if (props.allowInactiveSelection) {
        return !!account;
      }
      return (
        account &&
