                ...
                <div class="flex-1 min-h-0">
                    <div v-if="loading" class="flex flex-col items-center justify-center flex-1 border border-dashed border-emerald-300 rounded-lg py-10">
                        <div class="flex items-center space-x-3 text-emerald-600">
                            ...
                            <span class="text-sm font-medium">Loading glueing material data...</span>
                        </div>
                    ...
                    <div v-else class="overflow-x-auto rounded-lg border border-gray-200 max-h-96 flex-1 min-h-0">
                        <table class="w-full divide-y divide-gray-200 table-fixed min-w-[520px]">
                            <thead class="bg-gray-50 sticky top-0">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/3 cursor-pointer" @click="toggleSort('name')">Name</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-xs">
                                <tr v-if="filteredItems.length === 0">
                                    <td colspan="4" class="px-4 py-4 text-center text-gray-500">No data available.</td>
                                </tr>
                                <tr v-else v-for="item in filteredItems" :key="item.code"
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-gray-700 truncate max-w-[240px]">{{ item.description || '-' }}</td>
