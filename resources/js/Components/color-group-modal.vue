        ...
        <div class="flex-1 min-h-0">
          <div v-if="loading" class="flex flex-col items-center justify-center flex-1 border border-dashed border-emerald-300 rounded-lg py-10">
            <div class="flex items-center space-x-3 text-emerald-600">
              ...
              <span class="text-sm font-medium">Loading color group data...</span>
            </div>
          ...
          <table class="w-full divide-y divide-gray-200 table-fixed">
            <thead class="bg-gray-50 sticky top-0">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/5 cursor-pointer" @click="sortTable('cg')">CG#</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/5 cursor-pointer" @click="sortTable('cg_name')">CG Name</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/5 cursor-pointer" @click="sortTable('cg_type')">CG Type</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/5">Status</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200 text-xs">
              <tr v-for="group in filteredColorGroups" :key="group.cg"
                :class="['hover:bg-emerald-50 cursor-pointer', selectedGroup && selectedGroup.cg === group.cg ? 'bg-emerald-100 border-l-4 border-emerald-500' : '']"
                @click="selectRow(group)"
                @dblclick="selectAndClose(group)">
                <td class="px-6 py-3 whitespace-nowrap font-medium text-gray-900">{{ group.cg }}</td>
                <td class="px-6 py-3 whitespace-nowrap text-gray-700 truncate">{{ group.cg_name }}</td>
                <td class="px-6 py-3 whitespace-nowrap text-gray-700">{{ group.cg_type }}</td>
              ...
              <tr v-if="filteredColorGroups.length === 0">
                <td colspan="4" class="px-6 py-4 text-center text-gray-500">No color group data available.</td>
              </tr>
            </tbody>
          </table>
        </div>
