  ...
  <div v-if="show" class="fixed inset-0 z-50 overflow-y-auto">
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
      ...
      >
        <!-- Header -->
        <div
          class="px-8 py-5 border-b border-gray-200 bg-gradient-to-r from-green-600 to-emerald-600"
        >
        ...
        <div class="px-8 py-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <div class="mb-6">
            ...
            <!-- Totals Row -->
            <div
              class="bg-gray-50 px-4 py-3 border border-t-0 border-gray-200 rounded-b-lg"
            >
              <div class="flex items-center justify-between">
        ...
        <!-- Footer -->
        <div
          class="px-8 py-6 bg-gray-50 border-t border-gray-200 flex justify-end space-x-4"
        >
          <button
            @click="$emit('close')"
...
const normalizeDesignUnit = (unit) => {
  ...
  if (
    ...
    return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  // Legacy exact labels from older UI
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  ...
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  // Older value from prior dropdown
  if (u === "PCS" || u === "PCS" || u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  // Specific legacy option labels
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
  if (u === "PCS" || u === "PCS") return "PCS";
...
const totalUnitPrice = ref(3036.36);
...
const hydrateFromMcComponents = () => {
  try {
    ...
    comps.forEach((comp) => {
      ...
      const extParts = [extL, extW, extH].filter(
        (v) => v !== undefined && v !== null && v !== ""
      );
      const intParts = [intL, intW, intH].filter(
        (v) => v !== undefined && v !== null && v !== ""
      );
    ...
    console.log("✅ ProductDesignScreenModal hydrated from mcComponents:", {
      ...
      dimensionItems: dimensionItems.map((d) => ({
        name: d.name,
        ext: d.extDimension,
        int: d.intDimension,
        part: d.partNumber,
...
const setQuantity = () => {
  ...
  console.log(
    `Main item updated via setQuantity: quantity=${items[0].quantity}, unitPrice=${items[0].unitPrice}, amount=${items[0].amount}`
  );
};
...
const applyQuantityToAllComponents = (qty) => {
  ...
  items.forEach((item, index) => {
    ...
    if (index === 0) {
      ...
      console.log(
        `Main item set from external quantity: quantity=${item.quantity}, unitPrice=${item.unitPrice}, amount=${item.amount}`
      );
      return;
    }
    ...
    calculateAmount(item);
    console.log(
      `Fit item ${index} (${item.name}) set from external quantity: quantity=${item.quantity}, unitPrice=${item.unitPrice}, amount=${item.amount}`
    );
  });
};
...
const applyExternalQuantity = (qty) => {
  ...
  if (qty && Number(qty) > 0) {
    ...
    console.log(
      "External quantity applied to all components:",
      numericQty,
      "items:",
...
onMounted(() => {
  console.log(
    "ProductDesignScreenModal mounted with initialQuantity:",
    props.initialQuantity
  );

  // Ensure unit values are always restricted (KG/PCS)
  items.forEach((it) => {
    it.unit = normalizeDesignUnit(it.unit);
  });
  ...
  if (props.masterCard) {
    ...
    if (!items[0].unitPrice) {
      items[0].unitPrice = props.masterCard.unit_price || 3036.36;
    }
    console.log("Master card data loaded:", {
