<template>
  <div v-if="show" class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl w-11/12 md:w-2/3 lg:w-3/4 max-w-4xl mx-auto">
      ...
      <div class="p-2 overflow-y-auto" style="max-height: 55vh;">
        ...
        <div v-else-if="filteredAccounts.length === 0" class="p-4 text-amber-700 bg-amber-50 rounded border border-amber-200">
          <div v-if="searchQuery">
            ...
            <button @click="searchQuery = ''" class="text-blue-600 underline ml-1">Clear search</button>
          </div>
          <div v-else>
        ...
        <table v-else class="min-w-full text-xs border border-gray-300">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="account in filteredAccounts" 
                ...
                :class="{ 
                  'bg-blue-200 border-2 border-blue-500 shadow-sm': selectedAccount?.customer_code === account.customer_code,
                  'hover:bg-blue-100': selectedAccount?.customer_code !== account.customer_code
                }"
      ...
      <!-- Modal Footer -->
      <div class="flex items-center justify-between gap-2 p-2 border-t border-gray-200 bg-gray-100 rounded-b-lg">
        <div class="text-xs text-gray-500 flex-1">
          <div v-if="filteredAccounts.length > 0">{{ filteredAccounts.length }} accounts found</div>
          <div v-if="selectedAccount" class="text-blue-600 font-medium mt-1">
            Selected: {{ selectedAccount.customer_code }} - {{ selectedAccount.customer_name }}
          </div>
        </div>
        <div class="flex gap-2">
          <button 
            ...
            :class="{
              'bg-blue-500 hover:bg-blue-700': selectedAccount && !loading,
              'bg-gray-400 cursor-not-allowed': !selectedAccount || loading
            }"
            ...
            {{ loading ? 'Loading...' : (selectedAccount ? 'Select' : 'Select a Customer') }}
          </button>
