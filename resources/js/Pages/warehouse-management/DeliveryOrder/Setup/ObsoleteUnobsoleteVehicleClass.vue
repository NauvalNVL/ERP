            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-lg sm:text-xl font-semibold leading-tight">
                  Manage Vehicle Class Status (Obsolete/Unobsolete)
                </h2>
                <p class="text-xs sm:text-sm text-indigo-100">
                  Toggle the active status of vehicle classes.
                </p>
              </div>
            </div>
            <div class="text-xs sm:text-sm text-indigo-100 flex items-center gap-2">
              <i class="fas fa-info-circle text-sm"></i>
              <span>Use search and status filters to quickly find vehicle classes.</span>
            </div>
          </div>
        </div>
          ...
          <div class="px-4 py-3 sm:px-6 border-b border-gray-100 flex items-center">
            ...
            <div>
              ...
              <p class="text-xs text-slate-500">
                Search, filter, and mark vehicle classes as active or obsolete.
              </p>
            </div>
          </div>
            ...
            <div v-else class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
              <table class="min-w-full divide-y divide-gray-200 bg-white">
                <thead class="bg-slate-50">
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-indigo-700 uppercase tracking-wider">Class Code</th>
...
const fetchVehicleClasses = async () => {
  ...
  try {
    ...
    if (data && data.success) {
      ...
      showErrorAlert('Error loading vehicle classes' + (data?.message ? `: ${data.message}` : ''))
      classes.value = []
      pagination.value = {
        currentPage: 1,
        perPage: 15,
    ...
    showErrorAlert('Error loading vehicle classes: ' + (error?.message || error))
    classes.value = []
    pagination.value = {
      currentPage: 1,
      perPage: 15,
...
const toggleClassStatus = async cls => {
  ...
  const result = await Swal.fire({
    ...
    text: `Vehicle class "${cls.VEHICLE_CLASS_CODE}" will be ${cls.STATUS === 'A' ? 'marked obsolete' : 'activated'}.`,
    icon: 'warning',
  ...
  try {
    ...
    showSuccessAlert(`Vehicle class "${cls.VEHICLE_CLASS_CODE}" successfully ${statusText}`)
  } catch (error) {
