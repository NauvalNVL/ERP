...
const toggleTaxGroupStatus = async (item) => {
  ...
  const result = await Swal.fire({
    title: currentStatus === "A" ? "Mark as Obsolete?" : "Activate Tax Group?",
  ...
  try {
    ...
    const response = await fetch(
      `/api/invoices/tax-groups/${encodeURIComponent(item.code)}/status`,
      {
        method: "PUT",
        headers: {
    ...
    toast.fire({
      icon: "success",
      title: `Tax group "${item.code}" ${statusText}`,
    });
  } catch (error) {
    ...
    Swal.fire({
      ...
      text: error.message || "Error updating tax group status.",
    });
  } finally {
