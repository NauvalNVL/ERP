                    ...
                    <div class="px-4 py-3 sm:px-6 flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-lg sm:text-xl font-semibold leading-tight">
                                Manage Tax Group Status
                            </h2>
                            <p class="text-xs sm:text-sm text-blue-100">
                                Toggle active/obsolete status of tax groups.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200">
                        <table class="min-w-full divide-y divide-gray-200 bg-white">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Group Code</th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Group Name</th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Sales Tax Applied</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr
                                    v-for="item in filteredTaxGroups"
                                    :key="item.code"
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-slate-700">{{ item.sales_tax_applied }}</td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center w-40">
                                        <button
                                            @click="toggleTaxGroupStatus(item)"
                                            :disabled="isToggling"
                                            :class="[
                                                getStatus(item) === 'A'
                                ...
                                <tr v-if="filteredTaxGroups.length === 0">
                                    <td colspan="5" class="px-4 py-8 text-center text-slate-500 text-sm">No tax groups found.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
const props = defineProps({
    ...
    header: {
        type: String,
        default: 'Manage Tax Group Status'
    }
});

const pageHeader = computed(() => props.header || 'Manage Tax Group Status');
...
const filteredTaxGroups = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        list = list.filter((item) => {
            ...
            const salesTax = (item.sales_tax_applied || '').toString().toLowerCase();
            return code.includes(q) || name.includes(q) || salesTax.includes(q);
        });
    }
...
const toggleTaxGroupStatus = async (item) => {
    if (isToggling.value) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/api/invoices/tax-groups/${encodeURIComponent(item.code)}/status`, {
            method: 'PUT',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData?.message || 'Failed to update tax group status');
        }

        item.status = newStatus;

        const statusText = newStatus === 'A' ? 'activated' : 'marked obsolete';
        showNotification(`Tax group "${item.code}" successfully ${statusText}`, 'success');
    } catch (error) {
        console.error('Error toggling tax group status:', error);
