<template>
  <AppLayout :header="'Define Tax Group'">
              ...
              <div class="px-4 py-3 sm:px-6 border-b border-gray-100 flex items-center">
                ...
                <div>
                  ...
                  <p class="text-xs text-slate-500">
                    Search, create, and maintain tax groups.
                  </p>
                </div>
              </div>
                  ...
                  <div class="col-span-2">
                    ...
                    <div class="relative flex">
                      <input
                        ...
                        :class="{
                          'bg-gray-100 cursor-not-allowed': recordMode === 'review',
                        }"
                      />
                      <button
                        type="button"
    ...
    >
      ...
      >
        <div
          class="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-blue-600 text-white rounded-t-xl"
        >
          <div class="flex items-center">
            ...
            <h3 class="text-sm font-semibold">
              {{ isCreating ? "Create Tax Group" : "Edit Tax Group" }}
            </h3>
          </div>
          <button
            type="button"
    ...
    >
      <div
        class="w-12 h-12 border-4 border-solid border-purple-500 border-t-transparent rounded-full animate-spin"
      ></div>
    </div>
  </AppLayout>
</template>
...
const handleObsoleteSelection = (code, message) => {
  showErrorAlert(message || `Tax group ${code || ""} is obsolete and cannot be used.`);
  handleCancel();
};
  ...
  try {
    ...
    if (recordMode.value === "add" || !originalCode.value) {
      ...
      response = await axios.put(
        `/api/invoices/tax-groups/${originalCode.value}`,
        payload
      );
    }
    ...
    if (status === 422) {
      ...
      showErrorAlert(`Validation failed: ${errorMessages}`);
    } else {
...
const saveTaxGroupChanges = async () => {
  ...
  // Sync modal data to the main form
  form.value = {
  ...
  // Determine create vs update mode before saving
  ...
  if (!success) {
    // If save fails, keep modal open so the user can correct it
    return;
  }

  // Close Tax Group modal and continue to Tax Item Screen for this group
  showEditModal.value = false;
  openTaxItemScreen();
};
