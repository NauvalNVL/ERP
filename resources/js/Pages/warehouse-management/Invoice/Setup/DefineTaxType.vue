<template>
    <AppLayout :header="'Define Tax Type'">
        ...
        <div class="bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 p-6 rounded-t-lg shadow-lg overflow-hidden relative">
            ...
            <div class="flex items-center">
                <div class="bg-gradient-to-br from-pink-500 to-purple-600 p-3 rounded-lg shadow-inner flex items-center justify-center relative overflow-hidden mr-4">
                    ...
                    <i class="fas fa-percent text-white text-2xl z-10"></i>
                </div>
                <div>
                    <h2 class="text-2xl md:text-3xl font-bold text-white mb-1 text-shadow">Define Tax Type</h2>
                    <p class="text-blue-100 max-w-2xl">Create and manage sales tax types</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-b-lg shadow-lg p-6 mb-6 bg-gradient-to-br from-white to-indigo-50">
            <div class="relative">
                <div class="absolute top-0 right-0">
                    <span
                        :class="recordMode === 'add' ? 'bg-green-600' : 'bg-orange-500'"
            ...
            <!-- New Tax Type Form (only when code is new) -->
            <div v-if="showNewForm" class="mt-6">
                <div class="bg-white p-6 rounded-lg shadow-md border-t-4 border-emerald-500">
                    <div class="flex items-center mb-4 pb-2 border-b border-gray-200">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800">Add New Tax Type</h3>
                    </div>

                    <form @submit.prevent="saveTaxType" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Tax Apply</label>
                                <div class="flex items-center space-x-6">
                                    <label class="inline-flex items-center space-x-2">
                                        <input type="radio" value="Y" v-model="form.apply" />
                                        <span>Y-Yes</span>
                                    </label>
                                    <label class="inline-flex items-center space-x-2">
                                        <input type="radio" value="N" v-model="form.apply" />
                                        <span>N-No</span>
                                    </label>
                                </div>
                            </div>
                            <div>
                                ...
                                <input type="number" step="0.01" v-model.number="form.rate" class="block w-full px-3 py-2 rounded-md border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 form-input" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Custom Type</label>
                                <select v-model="form.custom_type" class="block w-full px-3 py-2 rounded-md border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 form-input">
        ...
        <!-- Tax Type Table Modal -->
        <div v-if="showTable" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fadeIn">
            <div class="bg-white rounded-lg shadow-xl w-5/6 max-w-5xl max-h-[80vh] flex flex-col animate-scaleIn">
                <div class="bg-gradient-to-r from-teal-600 via-cyan-600 to-blue-600 text-white px-6 py-4 rounded-t-lg flex justify-between items-center">
                    <div class="flex items-center space-x-3">
                        ...
                        <h3 class="text-lg font-semibold">Sales Tax Type Table</h3>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Apply</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tax Rate %</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Custom Type</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Select</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="tx in filteredTaxTypes" :key="tx.code" class="hover:bg-blue-50">
                                    <td class="px-6 py-3 whitespace-nowrap font-medium text-blue-700">{{ tx.code }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap text-gray-700">{{ tx.name }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ (tx.apply || tx.sales_tax_applied) === 'N' ? 'N' : 'Y' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ Number(tx.rate ?? tx.tax_rate ?? 0).toFixed(2) }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">{{ tx.custom_type || 'N-NIL' }}</td>
                                    <td class="px-6 py-3 whitespace-nowrap">
                                        <button @click="selectFromTable(tx)" class="px-3 py-1.5 rounded-md text-white bg-blue-600 hover:bg-blue-700 shadow">
                                            Select
                                        </button>
                                    </td>
                                </tr>
                                <tr v-if="!loading && filteredTaxTypes.length === 0">
                                    <td colspan="6" class="px-6 py-6 text-center text-gray-500">No tax types found</td>
                                </tr>
                                <tr v-if="loading">
...
// UI State
const recordMode = ref('new'); // 'new' | 'add'
...
const filteredTaxTypes = computed(() => {
    if (!search.value) return taxTypes.value;
    const q = search.value.toLowerCase();
    return taxTypes.value.filter(x =>
...
const selectFromTable = (tx) => {
    ...
    form.value.apply = (tx.apply || tx.sales_tax_applied) === 'N' ? 'N' : 'Y';
    form.value.rate = Number(tx.rate ?? tx.tax_rate ?? 0);
    form.value.custom_type = tx.custom_type || 'N-NIL';
    recordMode.value = 'new';
    showTable.value = false;
    toast.success(`Selected Tax Type: ${form.value.code}`);
};
...
const handleCodeInput = async () => {
    ...
    try {
        ...
        const res = await fetch(`/api/material-management/tax-types/${codeEnc}`, { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' });
        if (res.ok) {
            const data = await res.json();
            if (data && (data.code || data.name)) {
                form.value.name = data.name || '';
                form.value.apply = (data.apply || data.sales_tax_applied) === 'N' ? 'N' : 'Y';
                form.value.rate = Number(data.rate ?? data.tax_rate ?? 0);
                form.value.custom_type = data.custom_type || 'N-NIL';
                recordMode.value = 'new';
                return;
            }
        }
...
const loadTaxTypes = async () => {
    loading.value = true;
    try {
        const res = await fetch('/api/material-management/tax-types', { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' });
        if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
        const data = await res.json();
        taxTypes.value = Array.isArray(data?.data) ? data.data : (Array.isArray(data) ? data : []);
    } catch (e) {
        taxTypes.value = [];
    } finally {
        loading.value = false;
...
const saveTaxType = async () => {
    if (!form.value.code || !form.value.name) {
        toast.error('Please fill Tax Code and Tax Name');
        return;
    }
    try {
        const payload = {
            ...
            is_applied: form.value.apply === 'Y',
            rate: Number(form.value.rate || 0),
            tax_group_code: form.value.code, // follow existing API pattern used elsewhere
            custom_type: form.value.custom_type,
        };
        const res = await axios.post('/api/material-management/tax-types', payload, { headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' } });
        if (res.data) {
            toast.success('Tax Type saved');
            recordMode.value = 'new';
            await loadTaxTypes();
        }
    } catch (e) {
        toast.error(e?.response?.data?.message || 'Failed to save Tax Type');
    }
};
