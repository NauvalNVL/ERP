<template>
  <AppLayout header="Amend SO">
    <div class="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <div
        class="max-w-5xl mx-auto bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden"
      >
                ...
                <div>
                  ...
                  <div class="flex items-center space-x-2">
                    ...
                    <span class="text-xs text-gray-500"
                      >Leave blank for loose item order</span
                    >
                  </div>
                </div>
    ...
    <DeliveryLocationModal
      ...
      "
      :order-details="
        selectedSO && selectedSO._originalData
          ? {
              deliveryLocation: {
                orderBy: {
                ...
                shipTo: {
                  ...
                  name:
                    selectedSO._originalData.order_info.delivery_to ||
  ...
  methods: {
    ...
    async fetchMcComponentsForDesign() {
      try {
        ...
        if (!mcSeq || !customerCode) {
          console.warn(
            "Cannot fetch MC components for AmendSO Product Design Screen: missing mcardSeq or customerCode",
            {
              mcardSeq: mcSeq,
        ...
        if (!response.ok) {
          console.warn(
            "Failed to fetch MC components for AmendSO design:",
            response.statusText
          );
          this.mcComponentsForDesign = [];
          return;
    ...
    async handleSalesOrderSelect(selectedOrder) {
      try {
        ...
        const response = await fetch(
          `/api/sales-order/${selectedOrder.soNumber}/detail`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          if (result.success) {
            ...
            this.selectedSO = {
              ...
              orderMode:
                data.order_info.order_mode ||
                "D-Order by Customer + Delivery & Invoice to Customer",
              ...
              month: data.order_info.po_date
                ? new Date(data.order_info.po_date).toLocaleString("default", {
                    month: "short",
                  })
                : "",
              setQuantity:
                data.order_info.set_quantity || data.item_details.order_qty || "",
