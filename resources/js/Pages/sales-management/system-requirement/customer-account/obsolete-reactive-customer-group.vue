				...
				<div class="bg-blue-600 text-white shadow-sm rounded-xl border border-blue-700 mb-4">
					<div class="px-4 py-3 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
						...
						<div class="relative w-full sm:w-72">
							<span class="absolute inset-y-0 left-0 flex items-center pl-3 text-blue-100">
				...
				<div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
						...
						<div v-else class="overflow-x-auto">
							<table class="min-w-full table-auto divide-y divide-gray-200 text-sm">
								<thead class="bg-gray-50">
									<tr>
										<th scope="col" class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">
											Group Code
										</th>
										<th scope="col" class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">
											Description
										</th>
										<th scope="col" class="px-4 py-2 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide">
											Status
										</th>
										<th scope="col" class="px-4 py-2 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide">
											Action
								...
								<tbody class="divide-y divide-gray-100 bg-white">
									<tr v-for="group in filteredGroups" :key="group.group_code" class="hover:bg-gray-50">
										...
										<td class="px-4 py-2 whitespace-nowrap text-sm">
											<div class="flex items-center justify-center">
												...
												>
													<i :class="[getStatusLabel(group) === 'Active' ? 'fas fa-toggle-off' : 'fas fa-toggle-on', 'mr-1']"></i>
													{{ getStatusLabel(group) === 'Unknown' ? 'N/A' : (getStatusLabel(group) === 'Active' ? 'Mark Obsolete' : 'Mark Active') }}
												</button>
											</div>
										</td>
									</tr>
					...
					<div v-if="isToggling" class="fixed inset-0 z-50 bg-black bg-opacity-30 flex justify-center items-center">
						<div class="bg-white px-6 py-4 rounded-lg shadow-lg text-center">
							<div class="w-8 h-8 border-4 border-solid border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
...
const getStatusLabel = (group) => {
	...
	if (status) {
		...
		if (s === 'act' || s === 'active' || s === 'a' || s === 'y' || s === '1' || s === 'true') return 'Active';
		if (s === 'obs' || s === 'obsolete' || s === 'inactive' || s === 'i' || s === 'n' || s === '0' || s === 'false') return 'Obsolete';
	}

	if (group.is_active !== undefined && group.is_active !== null) {
		if (group.is_active === true || group.is_active === 1 || group.is_active === '1') return 'Active';
		if (group.is_active === false || group.is_active === 0 || group.is_active === '0') return 'Obsolete';
	}
...
const toggleStatus = async (group) => {
	...
	if (getStatusLabel(group) === 'Unknown') {
		showNotification('Status field for Customer Group is not available in current data source.', 'error');
		return;
	}
