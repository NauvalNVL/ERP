  ...
  <AppLayout :header="'Define Customer Alternate Address'">
      ...
      <div class="flex items-center">
        <div class="bg-gradient-to-br from-cyan-500 to-teal-600 p-3 rounded-lg shadow-inner flex items-center justify-center mr-4">
          <i class="fas fa-address-book text-white text-2xl z-10"></i>
              ...
              <div>
          <h2 class="text-2xl md:text-3xl font-bold text-white mb-1 text-shadow">Define Customer Alternate Address</h2>
          <p class="text-cyan-100">Kelola alamat alternatif customer dengan mudah</p>
              </div>
            </div>
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2">
        <div class="relative bg-white rounded-2xl shadow-xl border border-gray-200 p-8 animate-fade-in-up overflow-hidden">
          <div class="flex items-center mb-6">
            <div class="bg-gradient-to-br from-cyan-500 to-blue-600 p-3 rounded-lg mr-3">
              <i class="fas fa-map-marker-alt text-white text-2xl"></i>
            </div>
            <h2 class="text-xl font-bold text-gray-800">Customer Alternate Address Management</h2>
          </div>
          <form @submit.prevent="handleSave">
            <div class="flex items-start justify-between mb-6">
              ...
              <label class="flex items-center text-gray-700 font-semibold mb-2">
                <span class="inline-flex items-center justify-center w-7 h-7 bg-gradient-to-br from-cyan-400 to-blue-500 text-white rounded-lg mr-2">
                  <i class="fas fa-id-card"></i>
                        </span>
                Customer Code
              </label>
              <div class="flex">
                <input type="text" class="flex-1 border border-gray-300 rounded-l-lg px-3 py-2 focus:ring-2 focus:ring-cyan-400" v-model="customerCode" />
                <button type="button" class="bg-gradient-to-br from-cyan-400 to-blue-500 text-white px-3 rounded-r-lg hover:from-cyan-500 hover:to-blue-600 transition" @click="openCustomerLookup">
                ...
                <div>
              <label class="flex items-center text-gray-700 font-semibold mb-2">
                <span class="inline-flex items-center justify-center w-7 h-7 bg-gradient-to-br from-pink-400 to-purple-500 text-white rounded-lg mr-2">
              ...
              <div class="flex">
                <input type="text" class="flex-1 border border-gray-300 rounded-l-lg px-3 py-2 focus:ring-2 focus:ring-pink-400" v-model="deliveryCode" />
                    <button type="button" class="bg-gradient-to-br from-pink-400 to-purple-500 text-white px-3 hover:from-pink-500 hover:to-purple-600 transition rounded-none" @click="openDeliveryLookup">
                      <i class="fas fa-table"></i>
                </button>
                    <button type="button" class="bg-gradient-to-br from-pink-400 to-purple-500 text-white px-3 hover:from-pink-500 hover:to-purple-600 transition rounded-r-lg" @click="handlePreviewDeliveryCode">
            ...
            <div v-if="showDetailsSection">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
                ...
                <div>
                  ...
                  <input type="text" id="country" v-model="country" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>

                <!-- State -->
                <div>
                  <label for="state" class="block text-gray-700 font-semibold mb-2">State</label>
                  <input type="text" id="state" v-model="state" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>

                <!-- Town -->
                <div>
                  <label for="town" class="block text-gray-700 font-semibold mb-2">Town</label>
                  <input type="text" id="town" v-model="town" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>

                <!-- Town Section -->
                <div>
                  <label for="townSection" class="block text-gray-700 font-semibold mb-2">Town Section</label>
                  <input type="text" id="townSection" v-model="townSection" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
              </div>

              <!-- Bill To Section -->
              <div class="bg-white p-6 rounded-lg shadow-lg mb-6 border border-blue-200">
                <h3 class="text-lg font-bold text-blue-800 mb-4 flex items-center">
                  <i class="fas fa-receipt mr-2"></i>Bill To
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  <div>
                    <label for="billToName" class="block text-gray-700 font-semibold mb-2">Name</label>
                    <input type="text" id="billToName" v-model="billTo.name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                  </div>
                  <div class="md:col-span-2">
                    <label for="billToAddress" class="block text-gray-700 font-semibold mb-2">Address</label>
                    <textarea id="billToAddress" v-model="billTo.address" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300"></textarea>
                  </div>
                </div>
              </div>

              <!-- Ship To Section -->
              <div class="bg-white p-6 rounded-lg shadow-lg mb-6 border border-purple-200">
                <h3 class="text-lg font-bold text-purple-800 mb-4 flex items-center">
                  <i class="fas fa-shipping-fast mr-2"></i>Ship To
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  <div>
                    <label for="shipToName" class="block text-gray-700 font-semibold mb-2">Name</label>
                    <input type="text" id="shipToName" v-model="shipTo.name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" />
                  </div>
                  <div class="md:col-span-2">
                    <label for="shipToAddress" class="block text-gray-700 font-semibold mb-2">Address</label>
                    <textarea id="shipToAddress" v-model="shipTo.address" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300"></textarea>
                  </div>
                  <div>
                    <label for="contactPerson" class="block text-gray-700 font-semibold mb-2">Contact Person</label>
                    <input type="text" id="contactPerson" v-model="shipTo.contactPerson" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" />
                  </div>
                  <div>
                    <label for="telNo" class="block text-gray-700 font-semibold mb-2">Tel No</label>
                    <input type="text" id="telNo" v-model="shipTo.telNo" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" />
                  </div>
                  <div>
                    <label for="faxNo" class="block text-gray-700 font-semibold mb-2">Fax No</label>
                    <input type="text" id="faxNo" v-model="shipTo.faxNo" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" />
                  </div>
                  <div>
                    <label for="email" class="block text-gray-700 font-semibold mb-2">Email</label>
                    <input type="email" id="email" v-model="shipTo.email" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="mt-8 flex justify-end space-x-4">
                <button @click="handleSave(); console.log('Final Save Check:', { customerCode: customerCode.value, deliveryCode: deliveryCode.value })" class="action-btn bg-gradient-to-br from-green-500 to-green-700 hover:from-green-600 hover:to-green-800">
                    <i class="fas fa-save mr-2"></i>Save
                </button>
                <button @click="handleClear" class="action-btn bg-gradient-to-br from-red-500 to-red-700 hover:from-red-600 hover:to-red-800">
...
const showDetailsSection = computed(() => customerCode.value !== '' || deliveryCode.value !== '' || altAddressId.value !== null);
...
async function fetchCustomerAccounts() {
  try {
    ...
    if (Array.isArray(response.data)) {
      ...
      console.error('Unexpected data format for customer accounts:', response.data);
      addToast('Failed to fetch customer accounts due to unexpected data format.', 'error');
      customers.value = [];
    }
  } catch (error) {
    addToast('Failed to fetch customer accounts: ' + error.message, 'error');
  }
}
...
async function fetchGeoData() {
  try {
    ...
    if (Array.isArray(response.data)) {
      ...
      console.error('Unexpected data format for geo data:', response.data);
      addToast('Failed to fetch geo data due to unexpected data format.', 'error');
      geoData.value = [];
    }
  } catch (error) {
...
async function fetchAlternateAddress(code) {
  try {
    ...
    const response = await axios.get(`/api/customer-alternate-addresses/${code}`);
    ...
    if (Array.isArray(response.data) && response.data.length > 0) {
      ...
      addToast('Alternate address not found for this customer. Please fill in the details to create a new one.', 'info');
      console.log('No alternate address found, clearing only related fields.');
    ...
    addToast('Failed to fetch alternate address: ' + (error.response?.data?.message || error.message), 'error');
    console.log('Error fetching alternate address, clearing only related fields:', error);
    altAddressId.value = null;
    deliveryCode.value = '';
    country.value = '';
    state.value = '';
...
function handlePreviewDeliveryCode() {
  if (deliveryCode.value && !country.value && geoData.value.length > 0) {
    const selectedGeo = geoData.value.find(geo => geo.code === deliveryCode.value);
    if (selectedGeo) {
...
async function handleSave() {
  console.log('Attempting to save:', { customerCode: customerCode.value, deliveryCode: deliveryCode.value });
  ...
  try {
    ...
    if (altAddressId.value) {
      ...
      response = await axios.put(`/api/customer-alternate-addresses/${altAddressId.value}`, payload);
      addToast('Customer Alternate Address updated successfully!', 'success');
    } else {
      // Create new record
      response = await axios.post('/api/customer-alternate-addresses', payload);
      addToast('Customer Alternate Address saved successfully!', 'success');
    }
    console.log(response.data);
    handleClear(); // Clear form after successful save/update
  } catch (error) {
    console.error('Error saving customer alternate address:', error);
    addToast('Failed to save Customer Alternate Address: ' + (error.response?.data?.message || error.message), 'error');
  }
}
