<template>
  <AppLayout :header="'Obsolete/Reactive Customer Account'">
    ...
    <form ref="csrfForm" class="hidden">@csrf</form>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs sm:text-sm text-blue-100">
              Manage customer account status: Activate or Deactivate accounts
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
              <div class="col-span-2">
                <label class="flex items-center text-sm font-medium text-gray-700 mb-1">
                  <span
                    class="flex items-center justify-center h-6 w-6 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 text-white mr-2 shadow-md"
                  >
                ...
                <div class="relative flex">
                  <input
                    type="text"
                    v-model="searchQuery"
                    class="flex-1 min-w-0 block w-full px-3 py-2 rounded-l-md border border-gray-300 focus:ring-cyan-500 focus:border-cyan-500 transition-colors"
                  />
                  <button
                    type="button"
                    @click="openSortDialog"
                    class="inline-flex items-center px-3 py-2 border border-l-0 border-gray-300 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-r-md transition-colors transform active:translate-y-px"
                  >
              ...
              <div class="col-span-1">
                <label class="flex items-center text-sm font-medium text-gray-700 mb-1">
                  <span
                    class="flex items-center justify-center h-6 w-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 text-white mr-2 shadow-md"
                  >
                ...
                <button
                  type="button"
                  @click="performAction"
                  class="w-full flex items-center justify-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-xl font-bold shadow-lg transition-all duration-200 transform hover:scale-105 active:scale-95"
                >
            ...
            <div v-else-if="!selectedAccount" class="mt-4 bg-yellow-100 p-3 rounded">
              ...
              <p class="text-xs text-yellow-700 mt-1">
                Please search and select a customer account first.
              </p>
            </div>
            <div v-else class="mt-4 bg-green-100 p-3 rounded">
              <p class="text-sm font-medium text-green-800">
                Selected Account: {{ selectedAccount.customer_code }} -
                {{ selectedAccount.customer_name }}
              </p>
              <div class="flex items-center mt-2">
                <span class="text-xs font-medium mr-2">Current Status:</span>
                <span
                  v-if="isStatusActive(selectedAccount.status)"
                  class="px-2 py-1 text-xs font-medium bg-green-200 text-green-800 rounded-full"
                >
            ...
            <div v-if="selectedAccount" class="mt-6 border-t border-gray-200 pt-6">
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p class="font-medium">
                    {{ selectedAccount.contact_person || "Not specified" }}
                  </p>
                </div>
                <div>
                  <p class="text-sm text-gray-600">Telephone:</p>
                  <p class="font-medium">
                    {{ selectedAccount.telephone_no || "Not specified" }}
                  </p>
                </div>
                <div>
                  <p class="text-sm text-gray-600">Email:</p>
                  <p class="font-medium">
                    {{ selectedAccount.co_email || "Not specified" }}
                  </p>
                </div>
              </div>
              <!-- Status Change Form -->
              <div class="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div class="mt-4">
                  <label class="text-sm font-medium text-gray-700 mb-1 flex items-center">
                    <span
                      class="flex items-center justify-center h-6 w-6 rounded-full bg-gradient-to-br from-pink-500 to-red-500 text-white mr-2 shadow-md"
                    >
                  ...
                  <textarea
                    v-model="reason"
                    rows="3"
                    class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
        ...
        <div class="lg:col-span-1">
          <!-- Info Card -->
          <div
            class="bg-white rounded-xl shadow-sm border border-blue-100 p-4 sm:p-5 mb-6"
          >
            ...
            <div class="space-y-4">
              <div class="p-4 bg-teal-50 rounded-lg">
                <h4
                  class="text-sm font-semibold text-teal-800 uppercase tracking-wider mb-2"
                >
                  Instructions
                </h4>
    ...
    <CustomerAccountModal
        ...
        :initial-status-filter="['all']"
        :allow-inactive-selection="true"
    ...
    >
      <div
        class="w-12 h-12 border-4 border-solid border-blue-500 border-t-transparent rounded-full animate-spin"
      ></div>
    </div>
    <!-- Notification Toast -->
    <div
...
const normalizeStatusValue = (status) => {
  ...
  if (['a', 'active', 'y', 'yes'].includes(normalized)) {
    return 'Active';
  }
  if (['i', 'inactive', 'n', 'no', 'obsolete'].includes(normalized)) {
    return 'Inactive';
  }
  return status;
};

const formatAccountRecord = (account = {}) => {
  const statusSource = account.status ?? account.AC_STS ?? account.active ?? account.AC_STS?.trim();
  return {
    ...account,
    status: normalizeStatusValue(statusSource)
  };
};

const isStatusActive = (status) => normalizeStatusValue(status) === 'Active';
...
const fetchCustomerAccounts = async () => {
  ...
  try {
    ...
    const response = await fetch("/api/customer-accounts?status=all", {
      headers: {
    ...
    if (Array.isArray(rawData)) {
      customerAccounts.value = rawData.map(formatAccountRecord);
      console.log(`Loaded ${rawData.length} customer accounts`);
    } else if (rawData.data && Array.isArray(rawData.data)) {
      customerAccounts.value = rawData.data.map(formatAccountRecord);
...
watch(searchQuery, (newQuery) => {
  if (newQuery && customerAccounts.value.length > 0) {
    ...
    if (foundAccount) {
      selectedAccount.value = formatAccountRecord(foundAccount);
      // Default to active if status is undefined
      newStatus.value = isStatusActive(foundAccount.status) ? 'Y' : 'N';
    }
  }
});

const onCustomerAccountSelected = (account) => {
  selectedAccount.value = formatAccountRecord(account);
  searchQuery.value = account.customer_code;
  showModal.value = false;
  // Default to active if status is undefined
  newStatus.value = isStatusActive(account.status) ? 'Y' : 'N';
  reason.value = '';
};
...
const confirmStatusChange = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `/api/customer-accounts/${selectedAccount.value.customer_code}/status`,
      {
        method: "PUT",
        headers: {
    ...
    // Update local data
    selectedAccount.value.status = newStatus.value === 'Y' ? 'Active' : 'Inactive';
...
const filteredCustomerAccounts = computed(() => {
  ...
  if (sortConfig.value.status === "active") {
    filtered = filtered.filter(
      (account) => account.status === "Active" || account.status === undefined
    );
