<template>
    <AppLayout :header="'Manage Industry Status'">
    <Head title="Manage Industry Status" />

    <!-- Header Section -->
    <div class="bg-gradient-to-r from-green-600 to-green-700 p-6 rounded-t-lg shadow-lg mb-6">
        <h2 class="text-2xl font-bold text-white mb-2 flex items-center">
            <i class="fas fa-sync-alt mr-3"></i> Manage Industry Status (Obsolete/Unobsolete)
        </h2>
        <p class="text-emerald-100">Toggle the active status of industries.</p>
    </div>
...
const props = defineProps({
    ...
    header: {
        type: String,
        default: 'Manage Industry Status'
    }
});
...
const fetchIndustries = async (page = 1) => {
    ...
    try {
        ...
        showNotification('Error loading industries: ' + error.message, 'error');
    } finally {
        loading.value = false;
    }
};

// Filter industries based on search query and status filter
