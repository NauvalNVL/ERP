  ...
  <AppLayout :header="'Salesperson'">
          ...
          <div class="px-4 py-3 sm:px-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs sm:text-sm text-green-100">
                Define salespersons for sales management
              </p>
            </div>
          </div>
        </div>
              ...
              >
                ...
                >
                  ...
                  <p class="text-xs text-yellow-700 mt-1">
                    Data will be automatically loaded when available.
                  </p>
                </div>
                <div v-else class="mt-4 bg-green-100 p-3 rounded">
                  <p class="text-sm font-medium text-green-800">
                    Data available: {{ salespersons.length }} salespersons found.
                  </p>
                ...
                <div class="space-y-4">
                  <div class="p-4 bg-teal-50 rounded-lg">
                    ...
                    <ul class="list-disc pl-5 text-sm text-gray-600 space-y-1">
                      ...
                      <li>
                        Use the <span class="font-medium">search</span> button to select a
                        salesperson
                      </li>
    ...
    >
      <div class="bg-white rounded-2xl shadow-xl w-11/12 md:w-2/5 max-w-md mx-auto">
        <div
          class="flex items-center justify-between p-4 border-b border-emerald-100 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-t-2xl"
        >
          <div class="flex items-center">
            ...
            <h3 class="text-xl font-semibold">
              {{ isCreating ? "Create Salesperson" : "Edit Salesperson" }}
            </h3>
          </div>
          <button
            type="button"
            @click="closeEditModal"
            class="text-white hover:text-emerald-100"
          >
          ...
          <form @submit.prevent="saveSalespersonChanges" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div class="relative flex">
                  <input
                    v-model="editForm.internal"
                    type="text"
                    class="flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-l-md border border-gray-300 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                  />
                  <button
                    type="button"
                    @click="openUserModal"
            ...
            <div class="flex justify-between mt-6 pt-4 border-t border-gray-200">
              <button
                type="button"
                v-if="!isCreating"
                @click="deleteSalesperson(editForm.id)"
              ...
              <div class="flex space-x-3">
                ...
                <button
                  type="submit"
                  class="px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-lg hover:from-emerald-600 hover:to-green-700"
                >
                  Save
                </button>
              </div>
...
const fetchUsers = async () => {
  ...
  try {
    ...
    users.value = rawUsers
      .map((u) => {
        const code = (u.code || u.user_id || u.userID || u.userId || "")
          .toString()
          .trim();
        const name = (
          u.name ||
...
watch(searchQuery, (newQuery) => {
  if (newQuery && salespersons.value.length > 0) {
    const foundPerson = salespersons.value.find(
      (person) =>
...
const deleteSalesperson = async (id) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      showNotification(
        "Error obsoleting salesperson: " + (result.message || "Unknown error"),
        "error"
      );
    }
  } catch (e) {
