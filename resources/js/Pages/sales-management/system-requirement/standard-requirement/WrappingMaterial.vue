  ...
  <AppLayout :header="'Define Wrapping Material'">
    ...
    <div class="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <div class="max-w-6xl mx-auto">
        <div
          class="bg-gradient-to-r from-green-600 to-green-700 text-white shadow-sm rounded-xl border border-green-700 mb-4"
        >
          <div class="px-4 py-3 sm:px-6 flex items-center gap-3">
            <div
              class="h-9 w-9 rounded-full bg-green-500 flex items-center justify-center"
            >
            ...
            <div>
              ...
              <p class="text-xs sm:text-sm text-green-100">
                Define wrapping materials used for product packaging
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-4 sm:p-6 mb-6">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2">
              <div
                class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border-t-4 border-emerald-500"
              >
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                  <div class="col-span-2">
                    ...
                    <div class="relative flex">
                      ...
                      <input
                        ...
                        placeholder="Search or type wrapping material code"
                      />
                      <button
                        type="button"
                  ...
                  <div class="col-span-1">
                    ...
                    <button
                      type="button"
                      @click="createNewWrappingMaterial"
                ...
                <div v-if="loading" class="mt-4 bg-yellow-100 p-3 rounded">
                  <div class="flex items-center">
                    <div class="mr-3">
                      <div
                        class="animate-spin rounded-full h-6 w-6 border-b-2 border-yellow-700"
                      ></div>
                    </div>
                    <p class="text-sm font-medium text-yellow-800">
                      Loading wrapping material data...
                    </p>
                  </div>
                </div>
                <div
                  v-else-if="wrappingMaterials.length === 0"
                  class="mt-4 bg-yellow-100 p-3 rounded"
                >
                  <p class="text-sm font-medium text-yellow-800">
                    No wrapping material data available.
                  </p>
                  <p class="text-xs text-yellow-700 mt-1">
                    Make sure the database is properly configured and seeders have been
                    run.
                  </p>
                </div>
                <div v-else class="mt-4 bg-green-100 p-3 rounded">
                  <p class="text-sm font-medium text-green-800">
                    Data available: {{ wrappingMaterials.length }} wrapping materials
                    found.
                  </p>
                  <p v-if="selectedRow" class="text-xs text-green-700 mt-1">
                    Selected: <span class="font-semibold">{{ selectedRow.code }}</span> -
            ...
            <div class="lg:col-span-1">
              <div
                class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border-t-4 border-teal-500 mb-6"
              >
                ...
                <div class="space-y-4">
                  ...
                  <div class="p-4 bg-blue-50 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 gap-2 text-sm">
                      <div class="flex items-center">
                        <span
                          class="w-6 h-6 flex items-center justify-center bg-blue-600 text-white rounded-full font-bold mr-2"
                          >SH</span
                        >
                      ...
                      <div class="flex items-center">
                        <span
                          class="w-6 h-6 flex items-center justify-center bg-purple-600 text-white rounded-full font-bold mr-2"
                          >PP</span
                        >
    ...
    >
      <div
        class="bg-white rounded-lg shadow-xl w-11/12 md:w-2/5 max-w-md mx-auto transform transition-transform duration-300"
      >
        <div
        ...
        >
          <div class="flex items-center">
            ...
            <h3 class="text-xl font-semibold">
              {{ isCreating ? "Create Wrapping Material" : "Edit Wrapping Material" }}
            </h3>
          </div>
          <button
            type="button"
            @click="closeEditModal"
            class="text-white hover:text-gray-200 transform active:translate-y-px"
          >
          ...
          <form @submit.prevent="savewrappingMaterialChanges" class="space-y-4">
            ...
            <div class="flex justify-between mt-6 pt-4 border-t border-gray-200">
              <button
                type="button"
                v-if="!isCreating"
                @click="deletewrappingMaterial(editForm.id)"
                class="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors text-sm transform active:translate-y-px"
              >
              ...
              <div class="flex space-x-3">
                ...
                <button
                  type="submit"
                  class="px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-lg hover:from-emerald-600 hover:to-green-700 transition-colors text-sm transform active:translate-y-px"
                >
    ...
    <div
      v-if="saving"
      class="fixed inset-0 z-[80] bg-black bg-opacity-50 flex justify-center items-center"
    >
      <div
        class="w-12 h-12 border-4 border-solid border-emerald-500 border-t-transparent rounded-full animate-spin"
      ></div>
    </div>
...
const fetchWrappingMaterials = async () => {
  ...
  try {
    ...
    wrappingMaterials.value = (list || []).filter(
      (item) => !item.status || item.status === "Act"
    );
  } catch (error) {
...
watch(searchQuery, (newQuery) => {
  if (newQuery && wrappingMaterials.value.length > 0) {
    const foundString = wrappingMaterials.value.find(
      (string) =>
...
const savewrappingMaterialChanges = async () => {
  ...
  try {
    ...
    let url = isCreating.value
      ? "/api/wrapping-materials"
      : `/api/wrapping-materials/${editForm.value.id}`;
    ...
    if (isCreating.value) {
      ...
      const newString = wrappingMaterials.value.find(
        (s) => s.code === editForm.value.code
      );
      if (newString) {
...
const deletewrappingMaterial = async (id) => {
  const confirmRes = await Swal.fire({
    title: "Obsolete Wrapping Material?",
    text: "Are you sure you want to obsolete this Wrapping Material?",
    icon: "warning",
