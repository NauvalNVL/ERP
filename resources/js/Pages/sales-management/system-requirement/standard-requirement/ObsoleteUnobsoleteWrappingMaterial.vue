        ...
        <div class="max-w-7xl mx-auto">
                ...
                <div class="px-4 py-3 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-emerald-100">Activate or deactivate wrapping materials quickly and easily.</p>
                        </div>
                    </div>
                    ...
                    <div class="relative w-full sm:w-72">
                        ...
                        <input
                            type="text"
                            v-model="searchQuery"
                            placeholder="Cari wrapping material (code, name)..."
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                <div class="px-4 py-2 sm:px-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white">
                    <div>
                        <h2 class="text-sm font-semibold text-gray-800">Wrapping Material List</h2>
                        <p class="text-xs text-gray-500">{{ filteredWrappingMaterials.length }} of {{ wrappingMaterials.length }} wrapping materials</p>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full table-auto divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                v-for="item in filteredWrappingMaterials"
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center w-40">
                                    <button
                                        @click="toggleWrappingMaterialStatus(item)"
                                        :disabled="isToggling"
                                        :class="[
                                            getStatus(item) === 'Act'
                            ...
                            <tr v-if="filteredWrappingMaterials.length === 0">
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">No wrapping materials found.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
const props = defineProps({
    ...
    header: {
        type: String,
        default: 'Manage Wrapping Material Status'
    }
});

const pageHeader = computed(() => props.header || 'Manage Wrapping Material Status');

const wrappingMaterials = ref(props.wrappingMaterials || []);
...
const filteredWrappingMaterials = computed(() => {
    let list = [...wrappingMaterials.value];
...
const toggleWrappingMaterialStatus = async (item) => {
    if (isToggling.value) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/api/wrapping-materials/${encodeURIComponent(item.code)}/status`, {
            method: 'PUT',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData?.message || 'Failed to update wrapping material status');
        }
        ...
        showNotification(`Wrapping material "${item.code}" successfully ${statusText}`, 'success');
    } catch (error) {
        console.error('Error toggling wrapping material status:', error);
