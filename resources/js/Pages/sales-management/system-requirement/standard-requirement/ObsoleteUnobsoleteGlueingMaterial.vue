        ...
        <div class="max-w-7xl mx-auto">
                ...
                <div class="px-4 py-3 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-emerald-100">Activate or deactivate glueing materials quickly and easily.</p>
                        </div>
                    </div>
                    ...
                    <div class="relative w-full sm:w-72">
                        ...
                        <input
                            type="text"
                            v-model="searchQuery"
                            placeholder="Cari glueing material (code, name)..."
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                <div class="px-4 py-2 sm:px-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white">
                    <div>
                        <h2 class="text-sm font-semibold text-gray-800">Glueing Material List</h2>
                        <p class="text-xs text-gray-500">{{ filteredGlueingMaterials.length }} of {{ glueingMaterials.length }} glueing materials</p>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full table-auto divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                v-for="item in filteredGlueingMaterials"
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center w-40">
                                    <button
                                        @click="toggleGlueingMaterialStatus(item)"
                                        :disabled="isToggling"
                                        :class="[
                                            getStatus(item) === 'Act'
                            ...
                            <tr v-if="filteredGlueingMaterials.length === 0">
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">No glueing materials found.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
const props = defineProps({
    ...
    header: {
        type: String,
        default: 'Manage Glueing Material Status'
    }
});

const pageHeader = computed(() => props.header || 'Manage Glueing Material Status');

const glueingMaterials = ref(props.glueingMaterials || []);
...
const filteredGlueingMaterials = computed(() => {
    let list = [...glueingMaterials.value];
...
const toggleGlueingMaterialStatus = async (item) => {
    if (isToggling.value) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/api/glueing-materials/${encodeURIComponent(item.code)}/status`, {
            method: 'PUT',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData?.message || 'Failed to update glueing material status');
        }
        ...
        showNotification(`Glueing material "${item.code}" successfully ${statusText}`, 'success');
    } catch (error) {
        console.error('Error toggling glueing material status:', error);
