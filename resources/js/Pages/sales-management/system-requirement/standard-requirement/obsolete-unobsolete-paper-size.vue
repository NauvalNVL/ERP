<template>
    ...
    <div class="bg-white rounded-b-lg shadow-lg p-6">
        <div v-if="notification.show" :class="{'bg-green-100 border border-green-400 text-green-700': notification.type === 'success', 'bg-red-100 border border-red-400 text-red-700': notification.type === 'error', 'px-4 py-3 rounded relative mb-4': true}">
        ...
        <div class="mb-6 flex flex-wrap items-center gap-4">
            <div class="flex-1 min-w-[300px]">
                <div class="relative">
                    ...
                    <input type="text" v-model="searchQuery" placeholder="Search paper sizes..." class="pl-10 pr-4 py-2 w-full border border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 bg-gray-50">
                </div>
            </div>
            <div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="size in filteredSizes" :key="size.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                            <span v-if="size.is_active" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                            <button @click="toggleSizeStatus(size)" :disabled="isToggling" :class="[size.is_active ? 'text-red-600 hover:text-red-900 bg-red-100 hover:bg-red-200' : 'text-green-600 hover:text-green-900 bg-green-100 hover:bg-green-200', 'transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 px-3 py-1 rounded text-xs font-semibold flex items-center justify-center']" :style="{ minWidth: '120px' }">
                                <i :class="[size.is_active ? 'fas fa-toggle-off' : 'fas fa-toggle-on', 'mr-1']"></i>
                                {{ size.is_active ? 'Mark Obsolete' : 'Mark Active' }}
                            </button>
                        </td>
                    </tr>
...
const props = defineProps({ paperSizes: { type: Array, default: () => [] }, pagination: { type: Object, default: () => ({ currentPage: 1, perPage: 15, total: 0 }) }, header: { type: String, default: 'Manage Paper Size Status' } });
...
const fetchPaperSizes = async () => {
    loading.value = true;
    try {
        const response = await fetch('/api/paper-sizes', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } });
        if (!response.ok) throw new Error('Failed to fetch paper sizes');
        paperSizes.value = await response.json();
    } catch (error) {
...
const filteredSizes = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        filtered = filtered.filter(size => String(size.millimeter).includes(query) || String(size.inches).includes(query));
    }
    if (statusFilter.value !== 'all') {
        const isActive = statusFilter.value === 'active';
        filtered = filtered.filter(size => size.is_active === isActive);
    }
    return filtered;
});
...
const toggleSizeStatus = async (size) => {
    ...
    if (!confirm(`Are you sure you want to change the status for "${size.millimeter} mm"?`)) return;
    isToggling.value = true;
    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
        if (!csrfToken) throw new Error('CSRF token not found');
        const response = await fetch(`/api/paper-sizes/${size.id}`, {
            method: 'PUT',
            headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json', 'Content-Type': 'application/json' },
            body: JSON.stringify({ is_active: !size.is_active })
        });
        if (!response.ok) throw new Error('Failed to toggle paper size status');
        size.is_active = !size.is_active;
        const statusText = size.is_active ? 'activated' : 'deactivated';
