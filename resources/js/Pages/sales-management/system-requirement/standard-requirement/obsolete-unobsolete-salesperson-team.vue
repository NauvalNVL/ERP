<template>
    <AppLayout :header="'Manage Salesperson Team Status'">
    <Head title="Manage Salesperson Team Status" />

    <!-- Header Section -->
    <div class="bg-gradient-to-r from-green-600 to-green-700 p-6 rounded-t-lg shadow-lg mb-6">
        <h2 class="text-2xl font-bold text-white mb-2 flex items-center">
            <i class="fas fa-sync-alt mr-3"></i> Manage Salesperson Team Status (Obsolete/Unobsolete)
        </h2>
        <p class="text-emerald-100">Toggle the active status of salesperson teams.</p>
    </div>
...
const props = defineProps({
    ...
    header: {
        type: String,
        default: 'Manage Salesperson Team Status'
    }
});

// Data
const salespersonTeams = ref(props.salespersonTeams || []);
...
const fetchSalespersonTeams = async (page = 1) => {
    ...
    try {
        const response = await fetch(`/api/salesperson-teams?page=${page}&all_status=1`, {
            headers: {
        ...
        showNotification('Error loading salesperson teams: ' + error.message, 'error');
    } finally {
        loading.value = false;
    }
};

// Filter salesperson teams based on search query and status filter
...
// Toggle salesperson team status
const toggleSalespersonTeamStatus = async (team) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to toggle salesperson team status');
        }
