        ...
        <div v-else class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
            <table class="min-w-full divide-y divide-gray-200 bg-white">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="flute in filteredFlutes" :key="flute.No" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                            <span v-if="flute.is_active" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                            <button @click="toggleFluteStatus(flute)" :disabled="isToggling"
                                :class="[
                                    flute.is_active
                                ...
                                <i :class="[flute.is_active ? 'fas fa-toggle-off' : 'fas fa-toggle-on', 'mr-1']"></i>
                                {{ flute.is_active ? 'Mark Obsolete' : 'Mark Active' }}
                            </button>
                        </td>
                    </tr>
...
const props = defineProps({
    paperFlutes: { type: Array, default: () => [] },
    pagination: { type: Object, default: () => ({ currentPage: 1, perPage: 15, total: 0 }) },
    header: { type: String, default: 'Manage Paper Flute Status' }
});
...
const fetchPaperFlutes = async () => {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Failed to fetch paper flutes');
        paperFlutes.value = await response.json();
    } catch (error) {
...
const filteredFlutes = computed(() => {
    ...
    if (statusFilter.value !== 'all') {
        ...
        filtered = filtered.filter(flute => flute.is_active === isActive);
    }
    return filtered;
});

const toggleFluteStatus = async (flute) => {
    if (isToggling.value) return;
    if (!confirm(`Are you sure you want to change the status for "${flute.Descr}"?`)) return;
    ...
    try {
        ...
        const response = await fetch(`/api/paper-flutes/${flute.No}`, {
            method: 'PUT',
            headers: {
            ...
            body: JSON.stringify({ is_active: !flute.is_active })
        });
        
        if (!response.ok) throw new Error('Failed to toggle paper flute status');
        
        flute.is_active = !flute.is_active;
        const statusText = flute.is_active ? 'activated' : 'deactivated';
        showNotification(`Paper Flute "${flute.Descr}" successfully ${statusText}`, 'success');
    } catch (error) {
