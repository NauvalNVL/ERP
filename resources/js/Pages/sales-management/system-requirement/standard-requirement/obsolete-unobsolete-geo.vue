<template>
	<AppLayout header="Manage Geo Status">
		<Head title="Manage Geo Status" />
					...
					<div class="px-4 py-3 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
						<div class="flex items-center gap-3">
							...
							<div>
								<h2 class="text-lg sm:text-xl font-semibold leading-tight">
									Manage Geo Status (Obsolete/Unobsolete)
								</h2>
								<p class="text-xs sm:text-sm text-emerald-100">
									Toggle the active status of geographic locations.
								</p>
							</div>
						</div>
						<div class="relative w-full sm:w-72">
							...
							<input
								v-model="searchQuery"
								type="text"
								placeholder="Search geo locations (code, country, town, area)..."
					...
					<div class="px-4 py-2 sm:px-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white">
						<div>
							...
							<p class="text-xs text-gray-500">
								{{ filteredGeos.length }} of {{ geos.length }} geo locations
							</p>
						</div>
...
// Filter geos based on search query and status filter
...
const toggleGeoStatus = async (geo) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to toggle geo status');
        }
