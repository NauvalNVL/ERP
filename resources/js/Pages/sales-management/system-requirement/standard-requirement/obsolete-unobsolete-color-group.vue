        ...
        <div class="max-w-7xl mx-auto">
            <div class="bg-emerald-600 text-white shadow-sm rounded-xl border border-emerald-700 mb-4">
                <div class="px-4 py-3 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-lg sm:text-xl font-semibold leading-tight">Manage Color Group Status</h1>
                            <p class="text-xs sm:text-sm text-emerald-100">Activate or deactivate color groups quickly and easily.</p>
                        </div>
                    </div>
                    ...
                    <div class="relative w-full sm:w-72">
                        ...
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Cari color group (code, name)..."
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                <div class="px-4 py-2 sm:px-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white">
                    <div>
                        <h2 class="text-sm font-semibold text-gray-800">Color Group List</h2>
                        <p class="text-xs text-gray-500">{{ filteredGroups.length }} of {{ colorGroups.length }} color groups</p>
                    </div>
                    ...
                    <div v-else class="overflow-x-auto">
                        <table class="min-w-full table-auto divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CG#</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CG Name</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CG Type</th>
                                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="group in filteredGroups" :key="group.cg" class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ group.cg }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                        <span v-if="group.status === 'Act'" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-emerald-100 text-emerald-800">
                                ...
                                <tr v-if="filteredGroups.length === 0">
                                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">No color groups found.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
const filteredGroups = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        filtered = filtered.filter(group => 
            (group.cg && group.cg.toLowerCase().includes(query)) || 
            (group.cg_name && group.cg_name.toLowerCase().includes(query)) ||
