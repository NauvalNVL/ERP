<template>
  <AppLayout :header="'Define Tax Group'">
    <Head title="Define Tax Group" />

    <!-- Header Section -->
    <div class="bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-t-lg shadow-md">
      <h2 class="text-2xl font-bold text-white mb-2 flex items-center">
        <i class="fas fa-layer-group mr-3"></i> Define Tax Group
      </h2>
      <p class="text-blue-100">Manage tax groups for material management</p>
    </div>
        ...
        <div class="flex-1">
          ...
          <div class="mb-6 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            ...
            <div class="flex gap-2 w-full md:w-auto">
              <button @click="newTaxGroup" class="btn-primary flex-1 md:flex-none">
                <i class="fas fa-plus-circle mr-2"></i> New Group
              </button>
              <button @click="refreshData" class="btn-secondary flex-1 md:flex-none">
                <i class="fas fa-sync-alt mr-2"></i> Refresh
              </button>
              <button @click="printTaxGroups" class="btn-info flex-1 md:flex-none">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  <tr v-else-if="paginatedTaxGroups.length === 0" class="hover:bg-gray-50">
                    <td colspan="2" class="px-6 py-4 text-center text-sm text-gray-500">
                      No tax groups found. Try adjusting your search.
                    </td>
                  </tr>
                  <tr v-for="group in paginatedTaxGroups" :key="group.code" 
                      @click="selectTaxGroup(group)" 
                      :class="{'bg-blue-50': selectedGroup && selectedGroup.code === group.code}"
          ...
          <div class="mt-4 flex justify-between items-center text-sm text-gray-600">
            <div>
              <span>Showing {{ paginatedTaxGroups.length }} of {{ filteredTaxGroups.length }} groups</span>
            </div>
        ...
        <div class="w-full md:w-96 bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div v-if="selectedGroup" class="mb-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-3 flex items-center">
              <i class="fas fa-info-circle mr-2 text-blue-500"></i> Tax Group Details
            </h3>
            <div class="space-y-3">
              ...
              <div class="flex justify-between border-b border-gray-200 pb-2">
                ...
                <span class="font-medium text-gray-900">{{ selectedGroup.name }}</span>
              </div>
            </div>
            <div class="mt-6 flex space-x-2">
              <button @click="editTaxGroup(selectedGroup)" class="flex-1 btn-blue">
          ...
          <div v-else class="flex flex-col items-center justify-center h-64 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-1">No Group Selected</h3>
            <p class="text-gray-500 mb-4">Select a tax group from the table to view details</p>
            <button @click="newTaxGroup" class="btn-primary">
              <i class="fas fa-plus-circle mr-1"></i> Create New Group
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg z-10 relative transform transition-all duration-300 ease-in-out">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-800 rounded-t-xl">
          <div class="flex items-center">
            ...
            <h3 class="text-2xl font-bold text-white">{{ isEditing ? 'Edit' : 'New' }} Tax Group</h3>
          </div>
        ...
        <div class="p-6 max-h-[70vh] overflow-y-auto">
          <form @submit.prevent="saveTaxGroup">
            <div class="space-y-6">
              <!-- Code -->
              <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                <label class="text-sm font-semibold text-gray-700 mb-2 flex items-center">
                  <i class="fas fa-hashtag text-blue-500 mr-2"></i>
                  Tax Group Code<span class="text-red-500">*</span>
                </label>
                <input type="text" v-model="formTaxGroup.code" required :disabled="isEditing"
                  maxlength="6"
              ...
              <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                <label class="text-sm font-semibold text-gray-700 mb-2 flex items-center">
                  ...
                  Tax Group Name<span class="text-red-500">*</span>
                </label>
                <input type="text" v-model="formTaxGroup.name" required
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
    ...
    <div v-if="showConfirmation" class="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div class="bg-white rounded-lg shadow-lg max-w-md z-10 w-full">
        <div class="p-6">
          ...
          <p class="mb-4 text-gray-600">Are you sure you want to delete the tax group <span class="font-semibold">{{ groupToDelete?.code }}</span>? This action cannot be undone.</p>
          <div class="flex justify-end space-x-3">
            ...
            <button @click="deleteTaxGroup" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700" :disabled="loading">
              <i class="fas fa-trash-alt mr-1"></i> Delete
            </button>
          </div>
        </div>
...
// Form for creating/editing tax group
const formTaxGroup = ref({
  code: '',
  name: '',
});
...
const paginatedTaxGroups = computed(() => {
  ...
  return filteredTaxGroups.value.slice(start, end);
});

const totalPages = computed(() => {
    const total = filteredTaxGroups.value.length;
    if (total === 0) return 1;
    return Math.ceil(total / itemsPerPage.value);
});

watch(filteredTaxGroups, () => {
...
const saveTaxGroup = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      response = await axios.put(`/api/material-management/tax-groups/${formTaxGroup.value.code}`, formTaxGroup.value);
      toast.success('Tax group updated successfully');
      
      const index = taxGroups.value.findIndex(d => d.code === formTaxGroup.value.code);
      if (index !== -1) {
        taxGroups.value[index] = response.data;
        selectedGroup.value = response.data;
      }
    } else {
      response = await axios.post('/api/material-management/tax-groups', formTaxGroup.value);
      toast.success('Tax group created successfully');
      taxGroups.value.push(response.data);
      selectedGroup.value = response.data;
    }
    ...
    toast.error(error.response?.data?.message || 'Failed to save tax group');
  } finally {
    loading.value = false;
  }
};
...
const deleteTaxGroup = async () => {
  ...
  loading.value = true;
  try {
    await axios.delete(`/api/material-management/tax-groups/${groupToDelete.value.code}`);
    toast.success('Tax group deleted successfully');
    
    taxGroups.value = taxGroups.value.filter(d => d.code !== groupToDelete.value.code);
    
    if (selectedGroup.value?.code === groupToDelete.value.code) {
      selectedGroup.value = null;
    }
