      ...
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-xl rounded-lg overflow-hidden">
          <div class="p-6">
            ...
            <!-- Table -->
            <div id="taxTypeTable" class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Code</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tax Apply</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  <tr v-for="row in filteredRows" :key="row.code" class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-gray-700">{{ row.rate.toFixed(2) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const printTable = () => {
  const printContents = document.getElementById('taxTypeTable').innerHTML;
...
const exportData = () => {
  ...
  csvContent += 'Code,Name,Tax Apply,%\n';
  filteredRows.value.forEach(row => {
    csvContent += `"${row.code}","${row.name}","${row.is_applied ? 'Yes' : 'No'}","${row.rate.toFixed(2)}"\n`;
  });
  const encodedUri = encodeURI(csvContent);
  const link = document.createElement('a');
  link.setAttribute('href', encodedUri);
  link.setAttribute('download', 'tax_types.csv');
