        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-800 flex items-center">
              <i class="fas fa-print text-orange-500 mr-3"></i>
              Print Return (RT)
            </h1>
            <p class="text-gray-600 mt-2">Generate and print return transaction reports</p>
          </div>
...
const generateReport = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      a.href = url
      a.download = `RT_Report_${form.value.date_from}_${form.value.date_to}.pdf`
...
const downloadReport = async (report) => {
  try {
    ...
    if (response.data.success) {
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `RT_Report_${report.id}.pdf`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      window.URL.revokeObjectURL(url)
