    ...
    <div class="bg-gradient-to-r from-indigo-600 to-indigo-800 rounded-lg shadow-lg p-6 mb-6 text-white">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-indigo-100 mt-2 text-lg">View and print debit note transaction logs and history</p>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <div class="text-sm text-gray-500">Total DN</div>
          </div>
        </div>
      </div>
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-800 flex items-center">
            ...
            Debit Note Log ({{ filteredNotes.length }} records)
          </h3>
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                <div>
                  ...
                  <div class="text-gray-500 text-xs">{{ note.vendor_name }}</div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <div>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          ...
          <button
            ...
            :key="page"
            @click="goToPage(page)"
            :class="page === currentPage ? 'bg-indigo-600 text-white' : 'text-gray-500 hover:text-gray-700'"
        ...
        <div class="mt-3">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900 flex items-center">
              ...
              Debit Note Details: {{ selectedNote?.note_number }}
            </h3>
            <button
          ...
          <div v-if="selectedNote" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div class="text-sm text-gray-900 p-2 bg-gray-50 rounded">{{ selectedNote.vendor_code || 'N/A' }}</div>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Vendor Name</label>
              <div class="text-sm text-gray-900 p-2 bg-gray-50 rounded">{{ selectedNote.vendor_name || 'N/A' }}</div>
            </div>
            <div>
...
const filteredNotes = computed(() => {
  let filtered = notes.value.filter(note => {
    ...
    if (filters.value.vendor) {
      matches = matches && (note.vendor_name && note.vendor_name.toLowerCase().includes(filters.value.vendor.toLowerCase()))
    }
...
const loadNotes = async () => {
  try {
    ...
    toast.error('Error loading debit note log')
  }
}
...
const printNote = (note) => {
  const printContent = `
    <div style="font-family: Arial, sans-serif; padding: 20px;">
      ...
      <p><strong>Vendor:</strong> ${note.vendor_name || 'N/A'}</p>
        ...
        <tbody>
          ${filteredNotes.value.map(note => `
            <tr>
              ...
              <td style="border: 1px solid #ddd; padding: 8px;">${note.vendor_name || 'N/A'}</td>
...
const exportLog = () => {
  const csvContent = "Note Number,Date,Vendor Code,Vendor Name,Reference,Amount,Status,Description,Created By\n" + 
    filteredNotes.value.map(note => 
      `${note.note_number},${note.note_date},${note.vendor_code || ''},${note.vendor_name || ''},${note.reference_document || ''},${note.amount},${note.status},"${note.description}",${note.created_by}`
    ).join('\n')
  ...
  a.href = url
  a.download = `DN_Log_${new Date().toISOString().split('T')[0]}.csv`
  a.click()
