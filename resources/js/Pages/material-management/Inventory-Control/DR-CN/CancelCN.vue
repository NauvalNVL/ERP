<template>
  <AppLayout>
    <!-- Header Section -->
    <div class="bg-gradient-to-r from-pink-600 to-pink-800 rounded-lg shadow-lg p-6 mb-6 text-white">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold flex items-center">
            <i class="fas fa-ban mr-3"></i>
            Cancel Credit Note (CN)
          </h1>
          <p class="text-pink-100 mt-2 text-lg">Search and cancel existing credit note transactions</p>
        </div>
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-800 flex items-center">
            ...
            Cancellable Credit Notes ({{ cancellableNotes.length }} records)
          </h3>
        ...
        <table class="w-full">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr 
              ...
              :key="note.id"
              :class="selectedNotes.includes(note.id) ? 'bg-pink-50' : 'hover:bg-gray-50'"
            >
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          ...
          <button
            ...
            :key="page"
            @click="goToPage(page)"
            :class="page === currentPage ? 'bg-pink-600 text-white' : 'text-gray-500 hover:text-gray-700'"
        ...
        <div class="mt-3 text-center">
          ...
          <div class="mt-2 px-7 py-3">
            <p class="text-sm text-gray-500">
              Are you sure you want to cancel the selected credit note(s)? This action cannot be undone.
            </p>
            <div class="mt-4">
              ...
              <textarea
                ...
                rows="3"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-pink-500 focus:border-transparent"
...
const confirmCancellation = async () => {
  ...
  try {
    ...
    if (successCount > 0) {
      toast.success(`${successCount} credit note(s) cancelled successfully!`)
      selectedNotes.value = []
      cancellationReason.value = ''
      showConfirmModal.value = false
      loadNotes()
    } else {
      toast.error('Failed to cancel credit notes')
    }
  } catch (error) {
    console.error('Error cancelling notes:', error)
    toast.error('Error cancelling credit notes')
  } finally {
