    ...
    <div class="bg-gradient-to-r from-purple-600 to-purple-800 rounded-lg shadow-lg p-6 mb-6 text-white">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-purple-100 mt-2 text-lg">Generate and print debit note reports and documents</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <input
            v-model="criteria.vendor_code"
            type="text"
            readonly
            @click="openVendorModal"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white cursor-pointer hover:bg-gray-50 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-purple-50 rounded-lg p-4">
          <div class="text-center">
            <div class="text-2xl font-bold text-purple-600">{{ reportData.total_count }}</div>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="note in reportData.notes" :key="note.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-sm text-gray-900">{{ note.vendor_name || 'N/A' }}</td>
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .bg-purple-50 { background-color: #f3e8ff; padding: 1rem; border-radius: 0.5rem; }
        </style>
      </head>
...
const downloadPDF = (data) => {
  ...
  a.href = url
  a.download = `DN_Report_${criteria.value.date_from}_to_${criteria.value.date_to}.json`
  a.click()
...
const downloadExcel = (data) => {
  // Mock Excel download
  const csvContent = "Note Number,Date,Vendor,Amount,Status,Description\n" + 
    data.notes.map(note => 
      `${note.note_number},${note.note_date},${note.vendor_name || ''},${note.amount},${note.status},"${note.description}"`
    ).join('\n')
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `DN_Report_${criteria.value.date_from}_to_${criteria.value.date_to}.csv`
  a.click()
  window.URL.revokeObjectURL(url)
}
...
const selectVendor = (vendor) => {
  criteria.value.vendor_code = vendor.code
  criteria.value.vendor_name = vendor.name
...
const loadRecentReports = () => {
  recentReports.value = [
    {
      id: 1,
      title: 'Debit Note Summary Report',
    ...
    {
      id: 2,
      title: 'Debit Note Detailed Report',
