<template>
  <AppLayout :header="'Prepare IS (Inventory Setup)'">
    <Head title="Prepare IS (Inventory Setup)" />
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-4xl font-bold flex items-center mb-3">
              <i class="fas fa-plus-circle mr-4 text-blue-200"></i>
              Prepare IS (Inventory Setup)
            </h1>
            <p class="text-blue-100 text-xl font-medium">Create and manage inventory setup transactions</p>
...
const generateTransactionNumber = async () => {
  try {
    const response = await fetch('/api/is-mi-mo-lt/generate-number', {
      ...
      body: JSON.stringify({ transaction_type: 'IS' })
    })
...
const saveTransaction = async () => {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      if (data.success) {
        toast.success('IS transaction saved successfully')
        clearForm()
        ...
        Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'IS transaction has been saved successfully.',
...
const printTransaction = () => {
  ...
  window.open(`/material-management/inventory-control/is-mi-mo-lt/print-is?transaction=${form.value.transaction_number}`, '_blank')
}
...
const copyFromLastTransaction = async () => {
  try {
    const response = await fetch('/api/is-mi-mo-lt/is-transactions?per_page=1')
    if (response.ok) {
      const data = await response.json()
      if (data.success && data.data.data.length > 0) {
        ...
        toast.info('Copied from last IS transaction')
      }
    }
  } catch (error) {
...
const loadRecentTransactions = async () => {
  try {
    const response = await fetch('/api/is-mi-mo-lt/is-transactions?per_page=5')
    if (response.ok) {
      const data = await response.json()
      if (data.success) {
