<template>
  <AppLayout :header="'Cancel IS (Inventory Setup)'">
    <Head title="Cancel IS (Inventory Setup)" />
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-4xl font-bold flex items-center mb-3">
              ...
              Cancel IS (Inventory Setup)
            </h1>
            <p class="text-red-100 text-xl font-medium">Cancel inventory setup transactions</p>
          ...
          <div class="flex space-x-4">
            ...
            >
              ...
              {{ isCancelling ? 'Cancelling...' : 'Cancel IS' }}
            </button>
            <button
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
      <div class="flex items-center justify-between">
        ...
        <div class="bg-red-100 text-red-800 px-4 py-2 rounded-lg font-medium">
          Cancellation Mode
        </div>
      </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <div class="space-y-3 max-h-96 overflow-y-auto">
            <div
              ...
              :key="transaction.id"
              class="p-4 border border-gray-200 rounded-lg cursor-pointer hover:border-red-300 hover:bg-red-50 transition-all"
              :class="{ 'border-red-500 bg-red-50': selectedTransaction?.id === transaction.id }"
      ...
      <div class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow-lg p-8">
          ...
          <div v-if="selectedTransaction" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  :value="selectedTransaction.formatted_transaction_date"
                  type="text"
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50"
                  readonly
                />
              ...
              <div>
                ...
                <input
                  :value="selectedTransaction.formatted_quantity"
                  type="text"
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50"
                  readonly
                />
              ...
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Total Amount</label>
                <input
                  :value="formatCurrency(selectedTransaction.total_amount)"
                  type="text"
          ...
          <div v-else class="text-center py-12">
            ...
            <p class="text-gray-600 text-lg">Please select a transaction to cancel</p>
          </div>
        </div>
      </div>
    </div>
...
const cancelTransaction = async () => {
  ...
  if (selectedTransaction.value.status === 'Posted') {
    toast.error('Cannot cancel posted transactions')
    return
  }
    ...
    if (response.ok) {
      ...
      if (data.success) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'IS transaction has been cancelled successfully.',
...
const printTransaction = () => {
  ...
  window.open(`/material-management/inventory-control/is-mi-mo-lt/print-is?transaction=${selectedTransaction.value.transaction_number}`, '_blank')
}
...
onMounted(() => {
  ...
  window.cancelIsInterval = interval
  
  loadTransactions()
})

onUnmounted(() => {
  if (window.cancelIsInterval) {
    clearInterval(window.cancelIsInterval)
  }
})
</script>
