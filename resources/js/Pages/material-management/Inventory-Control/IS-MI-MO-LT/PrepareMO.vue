<template>
  <AppLayout :header="'Prepare MO (Material Order)'">
    <Head title="Prepare MO (Material Order)" />
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-4xl font-bold flex items-center mb-3">
              <i class="fas fa-plus-circle mr-4 text-green-200"></i>
              Prepare MO (Material Order)
            </h1>
            <p class="text-green-100 text-xl font-medium">Create and manage material order transactions</p>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <button
                  @click="showSkuModal = true"
                  class="absolute right-2 top-1/2 transform -translate-y-1/2 text-green-600 hover:text-green-800"
                >
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
            ...
            Recent MO Transactions
          </h3>
          <div class="space-y-3">
            <div
              v-for="transaction in recentTransactions"
              :key="transaction.id"
              class="p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-green-50 transition-colors"
...
const generateTransactionNumber = async () => {
  try {
    const response = await fetch('/api/is-mi-mo-lt/generate-number', {
      ...
      body: JSON.stringify({ transaction_type: 'MO' })
    })
...
const saveTransaction = async () => {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      if (data.success) {
        toast.success('MO transaction saved successfully')
        clearForm()
        ...
        Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'MO transaction has been saved successfully.',
...
const printTransaction = () => {
  ...
  window.open(`/material-management/inventory-control/is-mi-mo-lt/print-mo?transaction=${form.value.transaction_number}`, '_blank')
}
...
const copyFromLastTransaction = async () => {
  try {
    const response = await fetch('/api/is-mi-mo-lt/mo-transactions?per_page=1')
    if (response.ok) {
      const data = await response.json()
      if (data.success && data.data.data.length > 0) {
        ...
        toast.info('Copied from last MO transaction')
      }
    }
  } catch (error) {
...
const loadRecentTransactions = async () => {
  try {
    const response = await fetch('/api/is-mi-mo-lt/mo-transactions?per_page=5')
    if (response.ok) {
      const data = await response.json()
      if (data.success) {
...
onUnmounted(() => {
  if (window.moInterval) {
    clearInterval(window.moInterval)
  }
})
