<template>
  <AppLayout :header="'Amend Purchase Requisition'">
    <Head title="Amend Purchase Requisition" />

    <!-- Header Section -->
    <div class="bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-t-lg shadow-md">
      <h2 class="text-2xl font-bold text-white mb-2 flex items-center">
        <i class="fas fa-file-edit mr-3"></i> Amend Purchase Requisition
      </h2>
      <p class="text-blue-100">Modify existing purchase requisition details</p>
    </div>
          ...
          <div class="mb-6 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            <div class="flex-1 w-full">
              <div class="relative">
                ...
                <input type="text" v-model="searchQuery" placeholder="Search PR by number or department..."
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  <tr v-else-if="paginatedPRs.length === 0" class="hover:bg-gray-50">
                    <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">
                      No purchase requisitions found. Only Draft status PRs can be amended.
                    </td>
                  </tr>
        ...
        <div class="w-full lg:w-96 bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div v-if="selectedPR" class="mb-6">
            ...
            <div class="mt-6 space-y-2">
              <button v-if="selectedPR.status === 'Draft'" @click="amendPR(selectedPR)" class="w-full btn-blue">
                <i class="fas fa-edit mr-1"></i> Amend PR
              </button>
              <button v-else class="w-full btn-secondary cursor-not-allowed" disabled>
                <i class="fas fa-lock mr-1"></i> Cannot Amend ({{ selectedPR.status }})
              </button>
            </div>
          </div>
          <div v-else class="flex flex-col items-center justify-center h-64 text-center">
            <div class="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mb-4">
              <i class="fas fa-file-edit text-blue-500 text-2xl"></i>
            </div>
            <h3 class="text-lg font-medium text-gray-900 mb-1">No PR Selected</h3>
            <p class="text-gray-500 mb-4">Select a purchase requisition from the table to view details and amend</p>
            <div class="text-sm text-gray-400">
              <p>Only Draft status PRs can be amended</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md z-10 relative transform transition-all duration-300 ease-in-out">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-800 rounded-t-xl">
          <div class="flex items-center">
            <div class="bg-white/20 p-2 rounded-lg mr-3">
              <i class="fas fa-file-edit text-white text-xl"></i>
            </div>
        ...
        <div class="p-6">
          <div class="text-center mb-6">
            ...
            <h4 class="text-lg font-semibold text-gray-900 mb-2">Amend Purchase Requisition</h4>
            <p class="text-gray-600 mb-4">You are about to amend PR: <strong>{{ selectedPR?.pr_number }}</strong></p>
          </div>
          ...
          <div class="flex justify-end space-x-3">
            ...
            <button @click="confirmAmendment" :disabled="!amendmentReason.trim() || loading" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all duration-200 disabled:opacity-50">
...
const filteredPRs = computed(() => {
  ...
  let result = prs.filter(pr => pr.status === 'Draft') // Only Draft PRs can be amended
...
const amendPR = (pr) => {
  if (pr.status !== 'Draft') {
    toast.error('Only Draft purchase requisitions can be amended')
    return
  }
...
const confirmAmendment = async () => {
  ...
  try {
    ...
    await fetch(`/api/purchase-requisitions/${selectedPR.value.id}/amend`, {
      method: 'POST',
      headers: {
    ...
    toast.success('Amendment reason recorded. Redirecting to edit form...')
    showAmendModal.value = false
    
    // Redirect to prepare PR page with the selected PR for editing
    router.get(`/material-management/purchase-order/pr-po/prepare-pr?edit=${selectedPR.value.id}`)
