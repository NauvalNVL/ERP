      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-gradient-to-r from-teal-600 to-cyan-600 rounded-lg shadow-lg p-6 mb-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-white">View & Print Purchase AP Accounts</h1>
              <p class="text-teal-100 mt-1">View and generate reports for Purchase AP Accounts</p>
            </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
              ...
              <div class="flex items-center">
                ...
                <span class="text-sm text-gray-600">3 Vendors Configured</span>
              </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <div class="flex items-end">
              <button
                @click="applyFilters"
                class="w-full px-4 py-2 bg-teal-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-teal-700"
              >
                Apply Filters
              </button>
            </div>
          ...
          <div class="flex justify-between items-center">
            <div class="flex space-x-2">
              <button
                ...
                :disabled="loading"
                class="px-4 py-2 bg-teal-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-teal-700 disabled:opacity-50"
              >
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
          <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Purchase AP Accounts</h3>
          </div>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="item in filteredData" :key="item.vendor_code" class="hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ item.vendor_code }}</td>
...
const header = 'View & Print Purchase AP Accounts'
const title = 'View & Print Purchase AP Accounts - ERP System'

const loading = ref(false)
const apAccounts = ref([])
...
const filteredData = computed(() => {
  let filtered = apAccounts.value

  if (filters.vendor_code) {
    filtered = filtered.filter(item => 
      item.vendor_code.toLowerCase().includes(filters.vendor_code.toLowerCase())
    )
  }
...
const fetchApAccounts = async () => {
  try {
    const response = await fetch('/api/material-management/account/setup-account/purchase-ap-accounts')
    const data = await response.json()
    apAccounts.value = data
  } catch (error) {
    console.error('Error fetching AP accounts:', error)
    toast.error('Failed to load AP accounts')
  }
}
...
const generateReport = async () => {
  loading.value = true
  try {
    const response = await fetch('/api/material-management/account/setup-account/generate-purchase-ap-report', {
      method: 'POST',
      headers: {
