      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-gradient-to-r from-green-600 to-teal-600 rounded-lg shadow-lg p-6 mb-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-white">Confirm to Post RC</h1>
              <p class="text-green-100 mt-1">Confirm and post RC batches to accounts</p>
            </div>
          ...
          <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
              <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900">Post Confirmation</h3>
              </div>
              
              <div class="p-6">
                <form @submit.prevent="confirmToPost">
                  <!-- Period Information -->
                  ...
                  <!-- Batch Details (if selected) -->
                  <div v-if="selectedBatch" class="mb-6 p-4 bg-blue-50 rounded-lg">
                    <h4 class="text-sm font-medium text-gray-900 mb-3">Selected Batch Details</h4>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span class="font-medium text-gray-700">Batch Number:</span>
                        <span class="ml-2 text-gray-600">{{ selectedBatch.batch_number }}</span>
                      </div>
                      <div>
                        <span class="font-medium text-gray-700">Status:</span>
                        <span class="ml-2 text-gray-600">{{ selectedBatch.status }}</span>
                      </div>
                      <div>
                        <span class="font-medium text-gray-700">Start Note:</span>
                        <span class="ml-2 text-gray-600">{{ selectedBatch.start_note_number }}</span>
                      </div>
                      <div>
                        <span class="font-medium text-gray-700">End Note:</span>
                        <span class="ml-2 text-gray-600">{{ selectedBatch.end_note_number }}</span>
                      </div>
                      <div>
                        <span class="font-medium text-gray-700">Total Records:</span>
                        <span class="ml-2 text-gray-600">{{ selectedBatch.total_records }}</span>
                      </div>
                      <div>
                        <span class="font-medium text-gray-700">Prepared By:</span>
                        <span class="ml-2 text-gray-600">{{ selectedBatch.prepared_by }}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div class="p-6">
                <div class="space-y-3 text-sm text-gray-600">
                  <div class="flex items-start">
                    ...
                    <span>Only New batches can be posted.</span>
                  </div>
                  <div class="flex items-start">
                    <i class="fas fa-info-circle text-blue-500 mt-1 mr-2"></i>
                    <span>Posting will update RC notes to Posted status.</span>
                  </div>
                  <div class="flex items-start">
...
const header = 'Confirm to Post RC'
const title = 'Confirm to Post RC - ERP System'
...
const confirmToPost = async () => {
  if (!form.batch_number) {
    toast.error('Please select a batch to post')
    return
  }

  loading.value = true
  try {
    const response = await fetch('/api/material-management/accounts/rc-posting-batch/confirm-to-post', {
      method: 'POST',
      headers: {
    ...
    if (data.success) {
      ...
      toast.error(data.message || 'Failed to post batch')
    }
  } catch (error) {
