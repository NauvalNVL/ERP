    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (is_array($pd) && $hasPdPayload) {
                ...
                // Prefer explicit pd_setup.moreDescriptions array, fallback to root moreDescriptions[]
                $pdMoreDescriptions = isset($pd['moreDescriptions']) && is_array($pd['moreDescriptions'])
                    ? $pd['moreDescriptions']
                    : null;
                ...
                for ($i = 1; $i <= 5; $i++) {
                    ...
                    // 1) Try pd_setup.moreDescriptions[index]
                    if ($pdMoreDescriptions !== null) {
                        $val = $pdMoreDescriptions[$i - 1] ?? null;
                        if ($val !== null && $val !== '') {
                            $incoming = $val;
                        }
                    }

                    // 2) If still empty, fallback to root-level moreDescriptions array from validated payload
